...
pub fn model_settings(value: &SavedModelSettingsV1) -> Result<(), DomainError> {
    for (field, value) in [
        ...
        ("model_settings.saved_reasoning_effort", &value.saved_reasoning_effort),
    ] {
        if let Some(value) = value {
            text(value, 1, 200, false).map_err(|_| bad(field))?;
            if value.trim() != value { return Err(bad(field)); }
        }
    }
...
pub fn provider_url(value: &str) -> Result<(), DomainError> {
    ...
    if value.trim() != value || url.scheme() != "https" || url.host_str().is_none()
...
pub fn fast_tier(model: &CodexAdvertisedModelV1) -> Result<String, DomainError> {
    ["priority", "fast"].into_iter()
        .find(|id| model.service_tiers.iter().any(|tier| tier.id == *id))
        .map(str::to_owned)
        .ok_or(DomainError::CapabilityUnavailable("unsupported_model_service_tier"))
}
...
pub fn probe_outcome(
    ...
    let CodexProbeOutcomeV1::Available { native_version, account, effective, models } = value else {
        return Ok(());
    };
    ...
    if account.requires_openai_auth && account.authentication_kind.is_none() {
        return Err(DomainError::CapabilityUnavailable("codex_authentication_required"));
    }
    if account.plan_type.is_some() != (account.authentication_kind == Some(CodexAuthenticationKind::Chatgpt)) {
        return Err(bad("account.plan_type"));
    }
    if let Some(plan) = &account.plan_type { text(plan, 1, 120, false).map_err(|_| bad("account.plan_type"))?; }
    let mut ids = BTreeSet::new();
    let mut names = BTreeSet::new();
    for item in models {
        let model = &item.capability;
        for value in [&model.id, &model.model, &model.display_name, &model.default_reasoning_effort] {
            text(value, 1, 200, false).map_err(|_| bad("native_catalog.model"))?;
        }
        if !ids.insert(&model.id) || !names.insert(&model.model)
        ...
        for effort in &model.supported_reasoning_efforts {
            text(&effort.reasoning_effort, 1, 200, false).map_err(|_| bad("native_catalog.effort"))?;
            if effort.description.chars().count() > 8192 || !efforts.insert(&effort.reasoning_effort) {
                return Err(bad("native_catalog.effort"));
            }
        }
        if !efforts.contains(&model.default_reasoning_effort) { return Err(bad("native_catalog.effort")); }
        ...
        if item.default_service_tier.as_ref().is_some_and(|tier| !tiers.contains(tier)) {
            return Err(bad("native_catalog.service_tier"));
        }
    }
    ...
    let observed = models.iter().find(|item| item.capability.model == effective.model)
        .ok_or(DomainError::CapabilityUnavailable("effective_model_not_advertised"))?;
    if effective.reasoning_effort.as_ref().is_some_and(|value| !observed.capability.supported_reasoning_efforts.iter().any(|effort| effort.reasoning_effort == *value))
        || effective.service_tier.as_ref().is_some_and(|value| !observed.service_tiers.iter().any(|tier| tier.id == *value))
    ...
    if !settings.use_default_model_settings {
        if settings.saved_model.as_ref().is_some_and(|model| *model != effective.model)
            || settings.saved_reasoning_effort.as_ref().is_some_and(|effort| Some(effort) != effective.reasoning_effort.as_ref())
            || (settings.saved_fast_mode && effective.service_tier.as_ref() != Some(&fast_tier(observed)?))
        {
            return Err(DomainError::CapabilityUnavailable("codex_settings_not_honored"));
        }
    }
    Ok(())
}
