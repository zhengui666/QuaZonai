...
async fn alpha(pool: &PgPool, f: &Fixture) -> Alpha {
    ...
    sqlx::query("INSERT INTO app.alphas(id,project_id,name,lifecycle) VALUES($1,$2,'fixture','RESEARCH')")
        .bind(alpha.as_uuid()).bind(f.project.as_uuid()).execute(&mut *tx).await.unwrap();
...
async fn member(connection: &mut PgConnection, candidate: Id, alpha: &Alpha) -> Result<(), sqlx::Error> {
...
async fn target(connection: &mut PgConnection, candidate: Id, instrument: &str) -> Result<(), sqlx::Error> {
...
async fn candidate_members_cannot_borrow_qualified_alphas_from_another_project(pool: PgPool) {
    ...
    sqlstate(member(&mut tx, candidate, &foreign).await.unwrap_err(), "23503");
    ...
    tx.commit().await.unwrap();
    let count: i64 = sqlx::query_scalar("SELECT count(*) FROM app.candidate_alphas WHERE candidate_id=$1")
...
async fn candidate_membership_seals_at_commit_and_cannot_be_reopened(pool: PgPool) {
    ...
    sqlstate(member(&mut connection, candidate, &second).await.unwrap_err(), "23000");
    sqlstate(target(&mut connection, candidate, "SECOND.X").await.unwrap_err(), "23000");
    sqlstate(
        sqlx::query("DELETE FROM app.candidate_publications WHERE candidate_id=$1")
            .bind(candidate.as_uuid()).execute(&mut *connection).await.unwrap_err(),
        "23000",
    );
}

#[sqlx::test(migrations = "../../migrations")]
async fn an_evaluation_seals_candidate_members_even_before_the_creation_transaction_commits(pool: PgPool) {
    let f = fixture(&pool, budget()).await;
    ...
    sqlstate(target(&mut tx, candidate, "LATE.X").await.unwrap_err(), "23000");
    tx.rollback().await.unwrap();
}
...
async fn alpha_and_family_cannot_reset_the_project_lineage(pool: PgPool) {
    ...
    sqlx::query("INSERT INTO app.research_lineages(id,origin,reason) VALUES($1,'NEW','fixture')")
        .bind(new_lineage.as_uuid()).execute(&pool).await.unwrap();
    sqlstate(
...
#[sqlx::test(migrations = "../../migrations")]
async fn experiment_self_and_multirow_cycles_fail_while_ordinary_ancestry_is_retained(pool: PgPool) {
    let f = fixture(&pool, budget()).await;
    ...
    sqlstate(
        sqlx::query(insert).bind(self_parent.as_uuid()).bind(self_parent.as_uuid())
            .bind(a.experiment.as_uuid()).execute(&pool).await.unwrap_err(),
        "23514",
    );
    let child = Id::new();
    sqlx::query(insert).bind(child.as_uuid()).bind(a.experiment.as_uuid())
        .bind(a.experiment.as_uuid()).execute(&pool).await.unwrap();
    let left = Id::new();
    let right = Id::new();
    sqlstate(
...
async fn consumed_experiment_results_cannot_change_beneath_alpha_versions(pool: PgPool) {
    ...
    for change in [
        ...
        sqlstate(sqlx::query(&sql).bind(a.experiment.as_uuid()).execute(&pool).await.unwrap_err(), "23000");
    }
    ...
    sqlx::query("UPDATE app.experiments SET outcome='INCONCLUSIVE' WHERE id=$1")
        .bind(draft.as_uuid()).execute(&pool).await.unwrap();
}
...
async fn approval(pool: &PgPool, release: Id, policy: Id, downstream: Id, inputs: Id) -> Result<(), sqlx::Error> {
...
async fn policy_approval_is_bound_to_its_exact_mandate_project_and_downstream(pool: PgPool) {
    ...
    approval(&pool, release, policy, d, f.input_set).await.unwrap();
    sqlstate(approval(&pool, release, policy, other_d, f.input_set).await.unwrap_err(), "23503");
    let other = fixture(&pool, budget()).await;
    let (other_m, other_c, other_e) = portfolio(&pool, &other).await;
    let other_r = support::release(&pool, &other, other_m, other_c, other_e).await.unwrap();
    sqlstate(approval(&pool, other_r, policy, d, other.input_set).await.unwrap_err(), "23503");
}
...
async fn forward_window(pool: &PgPool, release: Id, evaluation: Id, inputs: Id) -> Result<(), sqlx::Error> {
...
async fn forward_evidence_cannot_borrow_another_subject_kind_or_input_snapshot(pool: PgPool) {
    ...
    let (mandate, candidate, evaluation) = portfolio(&pool, &f).await;
    let release = release(&pool, &f, mandate, candidate, evaluation).await.unwrap();
    sqlstate(forward_window(&pool, release, evaluation, f.input_set).await.unwrap_err(), "23503");
    ...
    forward_window(&pool, release, forward, inputs).await.unwrap();
    sqlstate(forward_window(&pool, release, forward, different_inputs).await.unwrap_err(), "23503");
    ...
    sqlstate(forward_window(&pool, release, wrong_subject, inputs).await.unwrap_err(), "23503");
}
...
async fn accepted_manifest_requires_exact_producer_and_cannot_be_replaced(pool: PgPool) {
    ...
    sqlstate(
        sqlx::query("UPDATE app.run_attempts SET accepted_at=statement_timestamp() WHERE id=$1")
            .bind(f.fence.attempt_id.as_uuid()).execute(&pool).await.unwrap_err(),
        "23514",
    );
    let mut tx = pool.begin().await.unwrap();
    ...
    for field in ["accepted_at", "result_manifest_artifact_id"] {
        ...
        sqlstate(sqlx::query(&sql).bind(f.fence.attempt_id.as_uuid()).execute(&pool).await.unwrap_err(), "23000");
    }
}
...
async fn session(connection: &mut PgConnection, f: &Fixture, run: Id, revision: i64) -> Result<(), sqlx::Error> {
...
async fn session_creation_freezes_only_the_current_locked_profile_revision(pool: PgPool) {
    ...
    let mut connection = pool.acquire().await.unwrap();
    sqlstate(session(&mut connection, &f, run, 99).await.unwrap_err(), "23503");
    session(&mut connection, &f, run, 1).await.unwrap();
    let old: serde_json::Value = sqlx::query_scalar("SELECT requested_settings FROM app.codex_sessions WHERE run_id=$1")
    ...
    sqlx::query("UPDATE app.codex_profiles SET saved_model='fixture-change' WHERE id=$1")
        .bind(f.profile.as_uuid()).execute(&pool).await.unwrap();
    let next = new_mission(&pool, &f).await;
    sqlstate(session(&mut connection, &f, next, 1).await.unwrap_err(), "23503");
...
async fn event_and_cursor_commit_or_rollback_together_without_gaps(pool: PgPool) {
    ...
    sqlstate(
        sqlx::query("UPDATE app.runs SET last_event_seq=1 WHERE id=$1")
            .bind(f.run.as_uuid()).execute(&pool).await.unwrap_err(),
        "23000",
    );
