...
use crate::{
    authority::Actor, commands, control::page, db, settings::read_authority, Store, StoreError,
};
...
impl Store {
    ...
    pub async fn codex_profiles(
        &self,
        actor: &Actor,
        query: &ListQuery,
    ) -> Result<Page<CodexProfileViewV1>, StoreError> {
    ...
    pub async fn codex_profile(
        &self,
        actor: &Actor,
        id: Id,
    ) -> Result<CodexProfileViewV1, StoreError> {
        let mut tx = self.pool.begin().await?;
        read_authority(&mut tx, actor).await?;
    ...
    pub async fn create_codex_profile<F, Fut>(
        ...
        actor: &Actor,
        key: &str,
        request: &CodexProfileCreateV1,
        verify: F,
    ) -> Result<CommandResult<CodexProfileViewV1>, StoreError>
    where
        F: FnOnce(CodexBindingCheck) -> Fut,
        Fut: Future<Output = Result<(), StoreError>>,
    {
        ...
        let prepared = commands::operator(
            &mut tx,
            actor,
            OperatorOperation::CodexProfileCreate,
            key,
            None,
            db::json(request)?,
        )
        ...
        let occupied: bool = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM app.codex_profiles WHERE codex_home_ref=$1)",
        )
        .bind(&request.home_binding)
        .fetch_one(&mut *tx)
        .await?;
        if occupied {
        ...
        Fut: Future<Output = Result<(), StoreError>>,
    {
        ...
        let prepared = commands::operator(
            &mut tx,
            actor,
            OperatorOperation::CodexProfileUpdate,
            key,
            Some(id),
            db::json(request)?,
        )
        ...
        account::no_active_account_operation(&mut tx, id).await?;
    ...
    pub async fn prepare_codex_probe(
        &self,
        actor: &Actor,
        key: &str,
        request: &CodexProbeRequestV1,
    ) -> Result<CodexProbePreparation, StoreError> {
        let mut tx = self.pool.begin().await?;
        let prepared = commands::operator(
            &mut tx,
            actor,
            OperatorOperation::CodexProbe,
            key,
    ...
    pub async fn complete_codex_probe(
        &self,
        ticket: CodexProbeTicket,
        outcome: CodexProbeOutcomeV1,
    ) -> Result<CommandResult<CodexProbeViewV1>, StoreError> {
        let mut tx = self.pool.begin().await?;
        let prepared = commands::operator(
            &mut tx,
            &ticket.actor,
            OperatorOperation::CodexProbe,
            &ticket.key,
            Some(ticket.request.profile_id),
        ...
        if !account::observation_after_account_operations(&mut tx, profile.id, ticket.started_at)
            .await?
        {
            return Err(StoreError::Conflict);
        }
        if profile.revision != ticket.request.expected_revision {
            return Err(StoreError::RevisionConflict {
                current: profile.revision,
            });
        }
    ...
    pub async fn codex_observation(
        &self,
        actor: &Actor,
        id: Id,
    ) -> Result<CodexObservationV1, StoreError> {
        let mut tx = self.pool.begin().await?;
        read_authority(&mut tx, actor).await?;
        ...
        let now: DateTime<Utc> = sqlx::query_scalar("SELECT clock_timestamp()")
            .fetch_one(&mut *tx)
            .await?;
        let account_unchanged = if let Some(value) = &recent {
            account::observation_after_account_operations(&mut tx, id, value.observed_at).await?
        } else {
            true
        };
        let state = match &recent {
            ...
            Some(value)
                if value.profile_revision != profile.revision
