...
impl Store {
    pub async fn invalidate_restored_access(
        ...
        sqlx::query("LOCK TABLE app.operator_auth_state,app.browser_logins,app.trusted_devices,app.operator_command_grants,app.machine_credentials,app.machine_credential_revocations IN SHARE ROW EXCLUSIVE MODE").execute(&mut *tx).await?;
        ...
        sqlx::query("UPDATE app.operator_auth_state SET session_epoch=$1,last_accepted_totp_step=greatest(coalesce(last_accepted_totp_step,0),floor(extract(epoch FROM clock_timestamp())/30)::bigint) WHERE singleton").bind(epoch).execute(&mut *tx).await?;
