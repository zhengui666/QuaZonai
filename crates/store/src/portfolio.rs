...
pub(crate) fn risk_capability(
    ...
    let required = match content.risk_measure {
        AllocationRisk::Cvar => Some(("portfolio-cvar", "LINEAR_PROGRAM", "portfolio_cvar")),
        AllocationRisk::Variance if content.constraints.max_ex_ante_risk.is_some() => Some((
    ...
    if let Some((adapter, cone, error)) = required {
        if cap.engine_versions.get(adapter).map(String::as_str) != Some("1")
            || !cap.solver_capabilities.iter().any(|v| v == cone)
        {
            return Err(domain::DomainError::CapabilityUnavailable(error));
        }
    }
    Ok(())
}
