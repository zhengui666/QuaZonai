...
impl Store {
        ...
        Published: std::future::Future<Output = Result<(), StoreError>>,
    {
        ...
        let (mut tx, run, window) = admit_build(tx, request, read, publish, "OPERATOR").await?;
...
struct BuildWindow {
    decision_asof: DateTime<Utc>,
    assumptions_input: Id,
    weights_deadline: DbCounter,
...
// Both callers own an authorized transaction; this core never grants authority.
async fn admit_build<'a, R, Read, P, Published>(
    mut tx: Tx<'a>,
    request: &PortfolioBuildRequestV1,
    mut read: R,
    mut publish: P,
    created_by: &str,
) -> Result<(Tx<'a>, RunSnapshotV1, BuildWindow), StoreError>
where
    R: FnMut(Id, DbCounter) -> Read,
    Read: std::future::Future<Output = Result<Vec<u8>, StoreError>>,
    P: FnMut(NativeObjectPublication) -> Published,
    Published: std::future::Future<Output = Result<(), StoreError>>,
{
                ...
                available_notional: liquidity
                    ...
                    .map(|source| {
                        source
                            ...
                            .iter()
                            .find(|q| {
                                q.dataset_revision_id == source.binding.source.dataset_revision_id
                            })
    ...
    let (decision_asof, target_until) = publication::target_window(
