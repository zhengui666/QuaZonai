...
WITH reachable_roles AS (
  ...
  SELECT oid, nspowner FROM pg_catalog.pg_namespace WHERE nspname = 'app'
)
SELECT NOT EXISTS (SELECT 1 FROM application_schema) OR EXISTS (
  ...
  WHERE r.rolsuper OR r.rolcreaterole OR r.rolcreatedb OR r.rolbypassrls OR r.rolreplication
     ...
     OR EXISTS (
       ...
       JOIN application_schema n ON n.oid = c.relnamespace
     ...
     OR EXISTS (
       ...
       JOIN application_schema n ON n.oid = p.pronamespace
       WHERE p.proowner = r.oid
     )
     OR EXISTS (
       SELECT 1 FROM pg_catalog.pg_type t
       JOIN application_schema n ON n.oid = t.typnamespace
