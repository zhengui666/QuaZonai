...
def test_candidate_strategy_wheel_uses_installable_pep427_filename() -> None:
    ...
    assert _strategy_wheel_filename(candidate_id) == filename
...
def test_alpha_qualification_replay_is_bound_to_full_request() -> None:
    ...
    changed_dataset = _qualification_contract(
        sealed_dataset_revision_id=uuid4(),
        name="Qualified alpha",
        role="PRIMARY_ALPHA",
    )
    with pytest.raises(QfError) as raised:
        _assert_qualification_replay(existing, changed_dataset)  # type: ignore[arg-type]
    assert raised.value.code == "ALPHA_QUALIFICATION_CONTRACT_REUSED"

    changed_role = _qualification_contract(
        sealed_dataset_revision_id=sealed_revision_id,
        name="Qualified alpha",
        role="HEDGE_ALPHA",
    )
    with pytest.raises(QfError) as raised_role:
        _assert_qualification_replay(existing, changed_role)  # type: ignore[arg-type]
    assert raised_role.value.code == "ALPHA_QUALIFICATION_CONTRACT_REUSED"
