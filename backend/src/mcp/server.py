"""OAuth-protected MCP Tool and Resource mapping for QuaZonai."""
...
def create_server(settings: McpGatewaySettings) -> ScopedMCPServer:
    server = ScopedMCPServer(
        name="QuaZonai",
        instructions=(
            "Operate the QuaZonai workstation through current Resources and scoped Tools. "
    ...
    @server.resource("quazonai://deployments/{resource_id}/universe")
    ...
    async def artifact_upload_route(request: Request) -> Response:
        ...
        try:
            offset = int(request.headers.get("x-quazonai-upload-offset", ""))
        except ValueError:
