...
```text
Human Web
  → password + TOTP / trusted-browser credential
  → FastAPI Core
...
- Operator Authentication 启用时，CLI 从环境读取 `QUAZONAI_API_TOKEN` 并以 `Authorization: Bearer` 调用 Operator API；
...
`QUAZONAI_API_TOKEN` 是本地 automation credential，不是网页登录 token。轮换该值后需要同步更新调用 CLI 的 shell/secret manager；不需要重新配置 Google Authenticator。
