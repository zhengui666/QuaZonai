...
首次安装在 `current` 已建立而 install 标记尚存时，仅以 `--no-recreate` 恢复既有容器和原 Worker 并补完激活，不再执行初始化、迁移或重写 Worker 单元。Node、Rust、Caddy、Debian 的全部 FROM 固定至已通过 Container 构建的 registry digest；更新这些构建输入需修改 Dockerfile 并重新验证，不引入额外发布服务或专项安全门禁。
