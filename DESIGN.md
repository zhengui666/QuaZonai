...
旧关系核对不仅依赖源库仍声明的约束：对支持的0029旧表，使用审查过的最终模型外键基线（含复合键）补充原生投影引用检查。缺少声明明确标记MISSING_DECLARED并要求复核；引用指向未投影或排除目标保留为未核验，不读取排除内容。可投影关系存在孤立引用时整批拒绝。结构名称改名不影响按源/目标列和MATCH语义匹配，同义重复约束不增加已检查数量。模型外键基线不等于完整多态关系或密封业务沿袭验收。
