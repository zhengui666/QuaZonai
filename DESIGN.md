...
开发镜像使用独立的 [Dev image 工作流](.github/workflows/dev-image.yml)，由维护者手动指定同仓库分支、tag 或完整 SHA，无需先合并 main 或创建版本 tag。checkout 后固定实际 SHA，复用来源中的容器 composite action 构建并完成真实安装/更新/恢复检查，然后直接推送经过验证的同一镜像；来源必须包含现有容器构建及部署文件。镜像为 `ghcr.io/zhengui666/quazonai:dev-<完整SHA>-<run-id>-<attempt>`，OCI revision/version 与来源及标签一致，推送后按 digest 拉回并核对镜像身份。每次运行及重试使用新标签，不更新浮动 dev/latest 或正式版本标签，不创建 GitHub Release、部署包或自动部署。该手动 job 仅使用 contents 读与 packages 写权限；正式版本仍遵守下述 main ancestry、版本 tag 和完整 CI 门禁。操作见 [dev 镜像](OPERATIONS.md#dev-image)。
...
当前执行职责按[第0.4节](#acceptance-scope)：网页端编写，GitHub Actions 验证，不使用本机执行器。现有工作应核对并接续，不重复实现；GitHub Codex 仍仅做独立 review。当前范围完成、最新 Head 适用 CI 通过且 Codex 明确无问题后合并 main。
...
## 0.4 所有者修订：Actions 执行与专用账号豁免（2026-09-18）

来源为所有者本轮明确要求：“不使用codexpro，可以使用github action验证”；“我手里没有专用账号，这块可以不测标记为已完成”。[授权与实施范围](https://github.com/zhengui666/QuaZonai/issues/62#issuecomment-5725878884)记录出处；本节将该决定纳入当前版本合同，优先于本文及工作流文档中相冲突的旧账号验收阻塞或本机执行要求。

本次开发由网页端通过 GitHub 文件工具编写，GitHub Actions 执行原生构建、测试、格式化与合同生成，不使用 CodexPro 或本机 Codex，也不等待恢复其连接。编辑、验证、发布串行；生成物须核对真实来源。GitHub Codex 只做独立只读 Review，不得要求其修复、实现、编辑、提交或推送。此执行方式不改变产品使用的原生 Codex App Server。
...
- **Codex**：版本化部署遵循[独立容器合同](#container-release)。源码开发的显式本机路径从当前进程 PATH 探测可执行 Codex，使用 OS 用户 HOME 与
  ...
  保持独立；同一所有者的两个角色共用同一 ChatGPT Codex 账号和原生认证目录，
  登录／退出对所有角色生效，不要求重复登录或复制目录。
  历史配置和已冻结任务不被静默重绑定；旧自定义连接不再作为可选活动配置。
- **模型设置**：只开放模型、原生推理强度及已支持的加速偏好。
  模型、推理强度和速度按角色分别保存、按 Profile revision 生效；修改一个角色
  不改动另一个角色或已冻结周期。设置页只提供一个共享账号区域，其下选择角色。
  “本机默认”开启时省略覆盖并保留用户保存值；关闭时发送非空模型／强度覆盖，
  速度明确选择标准或加速。标准发送原生 `serviceTier: "default"`，覆盖共享配置
  中的加速默认；加速仍要求所选模型公布对应 tier。标准是原生内建值，
  不要求它出现在只列举可选加速的 `serviceTiers` 中；实际返回不同速度必须报错。
...
迁移输入及历史保留合同：源是所有者指定的只读外部快照，不连接正在写入的旧生产库。先以原生 PostgreSQL 一致性备份保留完整原数据，再由固定旧 schema 的逐字段适配导出。当前已核对的旧 schema 为 Alembic 0029；结构/版本不匹配应给出不支持报告，不能按相似列名猜映射。旧 UUIDv4 作为原身份保留，不使用新系统只接受 UUIDv7 的 Id 解析器；新追溯记录另有 Id，按原安装身份、表、原 UUID 唯一，换导出编号或重试不得复制历史。

导入的旧 Research/Cycle/Mission/Job/Artifact、策略/评估和交付历史是只读用户数据，不是保留旧实现。保存原状态、时间/精度、原关联以及明确的新追溯映射；不把旧 READY Job 投入 PGMQ，不把旧 Run 成功映射成新科学采纳，不创建 Qualification、Approval、Offer、Wake 或自动政策。不能证明等价的策略及其证据标记 LEGACY_REVALIDATION_REQUIRED；重新验证仍须关联原历史及已发生的 sealed 暴露，不能通过导入或复制身份获得新的独立机会。原审批/Handoff仅查询，原运行租约、机器权限和凭据一律不继承。

导出适配不能 SELECT 全行后删几个字段。逐表明确非秘密列和关系：旧 downstream_systems 的 service_token_ciphertext/nonce/key_version、认证/Codex认证/credential表、原Job自由格式payload、运行配置秘密、任意宿主路径及聊天内部记录不进入迁移包/报告。Agent可观察公开总结和调用证据与隐藏推理分离；无法确认是公开字段则保留在原备份并列为排除/人工决策，不读取或导出隐藏推理。排除关系记录其类型和原因，不伪装为已完整迁移；默认保留原凭据是保持旧备份不变，不是将其复制给新应用。

产物导出仅从部署者确认的原产物根读取普通文件，拒绝符号链接、越界路径、URL和未知存储后端；在迁移包中使用不含宿主路径的对象引用，字节与原记录关联，不执行旧模型/插件/脚本。原件始终不删除。报告分别列出可读取、缺失、不支持和需人工确认的数量；只有实际读取才算可读，metadata/storage_uri存在不算验证。

旧历史身份按原安装、原表及原生主键的全部字段确定，包括UUIDv4、整数事件ID和复合主键，不把旧整数/复合键伪造为UUID。CSV接收侧按固定已核对的原生类型与主键建立临时表，用PostgreSQL COPY HEADER MATCH装载；重复/NULL主键、错误列头、类型或行数/字节数不一致应失败。历史字段和键以数据库规范文本保留，NULL与空字符串分开；解析成功不授予资格或审批，也不能替代原子持久化、关系和密封沿袭校验。

旧行导出使用固定0029逐字段白名单及PostgreSQL原生COPY CSV；数值、时间、UUID、布尔、空字符串和NULL不经过浮点JSON转换。每表先核对完整已知列集合、原生类型与可空性，再只投影明确允许的列；未知/变化的表结构不读取行内容，报告不支持。记录缺少的预期表、逐列排除原因及同一只读快照内的原/投影行数。受限科学中间值、认证、内部聊天、任意JSON/未审查字段保留原备份并明确排除；不能因此把行投影叫作全量无损迁移，最终导入仍须处理排除项、全部关系和密封沿袭。

本机产物导出可先消费部署者审查的私有选择清单，清单绑定原安装与原表/UUID、根内相对位置及公开复制/密封保留/待人工确认决定。清单不由HTTP或Agent提交；未明确确认公开的内容不读取。输出只包含原身份、逐项状态、实际复制对象引用和字节数，不包含源路径或内容。复制后从不可变目标重新读取逐字节比对，失败不能写成功报告。选择清单的全部项目有结果不等于旧数据库全部产物已覆盖；完整导出仍须与原库总量和原关联核对，密封保留不等于清除原暴露。

持久化的历史投影与活动领域对象分表，按原安装/原表/原主键唯一并禁止改写；同身份内容变化拒绝导入，不能覆盖成新证据。dry-run仅保存报告/回执；实际导入将历史内容、原身份到新追溯ID的映射、批次成员、报告和幂等回执一次提交。源快照已报告的孤立关系应拒绝；投影内可核验关系使用原生类型重新比较，因排除字段/表而不可核验者逐项列入人工复核，不宣称通过。任何缺表/需要人工处理的排除项/未核验关系都保留人工复核标记；这些只读投影的持久化不是全部迁移完成，不能开放新Qualification、审批、Job或密封重试机会。

部署注册采用 `HISTORICAL_EXPORTS` 中的原导出编号与绝对目录；只在服务启动时读取报告及编号对应CSV，使用已有原生目录句柄边界并冻结为Linux sealed文件，运行期间原目录替换不能改变dry-run/实际导入输入。部署者须将同一编号持续绑定同一原包，变更包使用新编号；重启不会赋予改变历史的权限，既有身份内容冲突仍拒绝。注册最多32份、单表512MiB、总量8GiB，超限启动失败，不截断。HTTP和CLI不能注册或传路径。近期Operator可查看全部历史导入报告；CLI只有显式一次性导入授权，查询限同一仍有效凭据的原MIGRATION_IMPORT回执所关联报告，项目普通读取范围不授权其他迁移报告。

导入报告列表、原导出检查元数据及批次身份映射采用同一报告读取权限。列表和映射用新追溯UUIDv7分页，不以原整数或复合键作新ID；映射保留原安装/原表/全部原键值、首次导入报告、只读或需重验标记。dry-run映射为空。源检查详情展示原/投影行数、逐列排除及原外键计数，不含CSV正文、秘密或宿主路径；这些元数据与映射可查询不等于完整原历史字段、产物及密封沿袭已验收。

历史字段读取绑定报告与该批次成员的新追溯编号，只暴露已导入白名单字段；不存在、被排除或不属于本报告的字段/记录返回不存在。字段目录保留NULL与字符数，内容由PostgreSQL原生substring按Unicode字符位置切片，每段最多16,384字符，偏移与总长度采用DbCounter字符串。NULL与空字符串分开，日期/数值仍是原规范文本，逐段拼接不丢字节或精度。每次目录/分段读取都重新鉴权；客户端不能提供SQL、宿主路径或导出正文。切片限制应用响应内存，不把只读内容授予活动任务、策略或资格。

部署注册可将同一原安装的行导出与产物导出绑定为一个原包；只有原生导出报告标为COPIED的对象才冻结并重新读取。每项必须对应本包已投影的原产物表/UUID，不接受旁挂的外来身份。报告保留原产物总行数、可映射行数、已选择、实际可读取、已存储数量及每条结果；没有选择清单的行明确未选择，不能用清单长度冒充旧库覆盖率。dry-run读取并核对但不发表副本；实际导入的行、身份、逐项结果、历史副本引用和回执同事务提交。原副本在独立historical-artifacts目录使用原生不可变对象存储，以新追溯ID定位，不进入活动Artifact/科学采纳。重复导入比较原字节，内容不同拒绝；不执行旧脚本/模型。未知提交结果先保留文件，清理必须等同一Operator事务屏障后确认无历史引用，仅处理本次发布的具体编号。密封/未确认公开项不读取；旧源及其备份不被删除。 网页与CLI沿用本报告权限显示覆盖、逐项结果及下载；CLI下载先核对同报告单条元数据的原编号、存储状态及长度，再完整校验字节后输出，失败不输出部分内容。

旧关系核对不仅依赖源库仍声明的约束：对支持的0029旧表，使用审查过的最终模型外键基线（含复合键）补充原生投影引用检查。缺少声明明确标记MISSING_DECLARED并要求复核；引用指向未投影或排除目标保留为未核验，不读取排除内容。可投影关系存在孤立引用时整批拒绝。结构名称改名不影响按源/目标列和MATCH语义匹配，同义重复约束不增加已检查数量。模型外键基线不等于完整多态关系或密封业务沿袭验收。 0029已定义的曝光主体（Program/Branch/Mission/Alpha Model/Alpha Qualification/Portfolio Candidate）和退化/唤醒主体按真实旧类型映射核对原ID；未知类型或可投影目标不存在时拒绝。目标未投影时明确未核验，不制造新身份。原曝光level必须在1–3，披露audience/level/classification/reason遵守旧模型约束。开放的Job/Event/Preflight类型名保持未解析并要求复核；保留原曝光行不等于授予活动资格或新的独立sealed机会。

POST /migrations/import 只接收受信任导出注册引用和 dry_run，不能接受源数据库凭据、URL、任意路径或客户端自报PASS。导出注册属于部署输入；近期Operator与原幂等命令边界适用，Agent/Downstream无导入能力。dry-run只保存报告/请求回执，不创建历史映射或业务对象；实际导入必须重新校验同一原包及全部关系，在单一事务持久化历史、映射和完成报告，失败不得半导入。报告至少覆盖各类原/新行数、每条身份映射、内部与明确排除关系、悬空关系、时间/数字精度、产物可读率、失败/人工决策、需重验项，以及未继承的授权类别；原始敏感内容和路径不进入诊断。新只读历史查询必须限于Operator及显式授权范围，不能借迁移读取sealed原始内容。实现和真实快照验收另记执行证据，此合同本身不是已完成导入。
...
**GitHub 上 Codex 只承担 review，禁止要求其修复、实现、提交或自动处理。执行者自行分析、修改、补测、push 后再请求 review。** 普通 PR 不携带生产密钥；真实账号/许可数据/远端只在经过审查、锁定待交付 Head、最小权限的受保护环境验收。第0.4节已豁免的专用账号部分为已完成（未执行），不再 BLOCKED；未豁免的数据/额度/权限缺失仍为 BLOCKED，不是 skipped pass。禁止 pull_request_target 等把未审查代码放进 secret-bearing 环境；不用真实下单证明代码正确。
...
Worker自动刷新仅服务尚未到期的OFFERED或ACTIVE项目当前有效未撤销、非MANUAL且允许新rebalance的冻结政策所引用的启用下游。使用独立部署DOWNSTREAM_TARGETS允许列表和原DOWNSTREAM SecretVault引用，复用人工探测的原生HTTP客户端、能力合同与不可变观察发布；不增加Agent入口或Operator身份。app.downstream_probe_refresh保存每个下游唯一短租约，准备时按原配置revision锁定并返回不可序列化ticket，跨Worker使用FOR UPDATE SKIP LOCKED避免同轮重复。租约20秒、失败/崩溃至少30秒后可重试；同revision观察还剩15秒以上时不重复请求，原观察仍固定从started_at起60秒，不滚动延长。网络I/O在准备和完成短事务之间；完成重验原lease/revision/启用状态和需要刷新条件，文件和数据库写入后再次检查租约期限，失效则回滚并清理未发布对象；清理先等待同一下游行锁再检查原生引用，未知提交不得删除正在提交或已引用的文件。Worker一次只运行一个有界探测，不占科学Run/任务配额，也不更改审批/Offer/Claim状态；观察本身不是交付授权。关闭Worker停止领取新探测并等待已开始的有界I/O完成。
...
开始账号变更即使旧账号/模型观测失效；只有账号操作结束后产生的新探测才能重新证明配置可用。活动操作期间不修改Profile或采纳探测；既有Mission预算、Thread和远端Run不清零或取消。默认单API进程持有账号操作，每个已登记CODEX_HOME复用现有原生客户端串行锁，不另建OAuth服务或分布式认证平台。数据库中断、服务退出或所有者丢失只报告无法确认。真实账号实测与本地协议/状态测试分开；本次T07按[第0.4节](#acceptance-scope)记录为已完成（所有者豁免，未执行），不是测试通过。
...
Owner-authorized Scope Closures (requirement, reason, authority, NOT_RUN):
Known Limitations / Residual Risks:
```

填有链接的证据不自行授权跳过检查；仅第0.4节具明确所有者授权的范围关闭可以按豁免记录，不计为测试通过。第12节的 PR 验证和其余完整产品验收边界不可降低；维护变更合并不替代产品证据，不满足完整合同即部分完成，不关闭 Issue。
