...
[公开历史数据准备](../runtimes/data/README.md)在操作边界固定数据集版本、许可与文件校验和，并按资产和时间转换链上成交、聚合 BAR 或盘口快照。下载器不依赖交易所；来源字段解释仅在历史适配器中。公开归档、事件时间与本地转换成功均不自动成为 PIT 或完整覆盖证据，也不扩展通用研究合同支持的数据类型。
