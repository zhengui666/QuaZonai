...
[GitHub review of `366feeeb`](https://github.com/zhengui666/QuaZonai/pull/84#discussion_r4028930351) identified a valid remaining CPU-fit problem. A short Mission created through draft-Brief updates reproduced it: after a real ledger lock, the old code persisted one CPU while the remaining window required two. Admission now freezes effective limits and deadline at the same database time; native task CPU uses those returned limits, and Runtime capacity checks use the effective allocation. New cases verify CPU/JobSpec agreement, complete rollback when capacity is insufficient, and rollback after real parent expiry without editing immutable records. All 46 experiment/qualification cases passed on the final local source (86.13 seconds), as did targeted Clippy. Independent rereading found and verified the fix for Reviewer's premature raw-wall capacity check. Full native Codex/OCI/browser coverage must pass again in the final Head's GitHub workflows; earlier runs are diagnostic evidence only.
