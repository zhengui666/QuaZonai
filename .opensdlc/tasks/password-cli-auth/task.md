...
The third review found that substring scanning rejected legitimate passwords matching public timestamp/ID prefixes. Typed scalar fields now reject complete original or normalized password reflections, while accepting prefixes; the returned public device name must equal the caller's input. Opaque token secrets and arbitrary error text retain substring protection. A fresh independent verifier found no further issue. Three focused unit checks and all 25 CLI integration tests passed, including real terminal login with a dynamically generated current-month-prefix password, successful saved identity and repeated login without an extra device.

Full hosted test logs also identified old anonymous-read expectations and historical migration fixtures calling current password admission against intentionally old schemas. Protected reads now assert 401 with no automatic session cookie; authenticated business checks remain. Historical fixtures construct actual pre-upgrade credential records and retain exact field-preservation and post-upgrade HTTP authority checks. All 18 artifact tests and 34 tests across the six affected HTTP suites passed locally. The portfolio suite passed 28 cases before its remaining anonymous-read assertion was corrected; the failed case then passed on rerun. Final server Clippy and formatting checks passed. Hosted final-Head results belong to the PR record.
