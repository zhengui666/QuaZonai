...
Current-Head review of `5398bbfdbd86ad6f3dede585d031e3a318454928` found a P1 operating-instruction defect (discussion `4068755893`): the results reference supplied parent Alpha IDs to version-scoped evaluations/qualifications/calibration reads. The correction uses `AlphaVersionView.id`, explicitly distinguishes it from `AlphaView.id` and the decimal version number, and gives the same version-ID contract to `alpha evaluate`. Native Clap value names/help now expose those distinctions without changing routing or authority. A binary-level regression extracts the actual documented command examples, substitutes different Alpha and version UUIDs, and checks their preview routes plus help labels. New-Head CI and a fresh clean review are required; no old clean verdict or in-progress check is reused.
