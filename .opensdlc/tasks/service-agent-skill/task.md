...
The bounded repair adds only that Demo evaluation's existing `EquityCurveV1` response, bound to its original project/Candidate/Evaluation/Run with `origin=FIXTURE` and `curve={status:UNAVAILABLE,reason_code:NO_SIMULATION}`. No snapshots, returns or production evidence are fabricated; unknown and Sealed evaluation IDs still return 404. The native contract unit test covers the exact payload and unavailable identities; browser history acceptance now awaits the curve response and verifies its unavailable UI/no chart before closing the dialog. All existing failure/write assertions remain. This aligns the Demo with the existing product contract, not a new API or scientific capability. New-Head Actions and read-only review are required again after publication; earlier passes do not transfer.
