...
### Revision, conflict and daily-budget parity

The [next review plan](https://github.com/zhengui666/QuaZonai/issues/62#issuecomment-5709361547) addresses comments 4033470215, 4033470219 and 4033470228. Native references are `Store::admit_cycle`, `apps/server/src/error.rs` and `formal_freeze_closes_brief_and_context_with_the_original_command_receipt` in the Store tests. Review/source inspection identified these defects; no separate pre-repair execution of the newly added assertions is claimed.

A successful freeze increments Brief revision with the existing BigInt/string helper, alongside its state/context and Project update. The receipt and subsequent GET share that revision; exact replay never increments again. For fresh Cycle requests, revision comparison precedes ACTIVE admission: stale requests receive 409/REVISION_CONFLICT with `current_revision`, and a current but inactive project receives 409/DOMAIN_CONFLICT. Freeze conflict responses also include the current revision. Tests assert these errors leave Cycle/Run/project state unchanged before an explicitly refreshed request succeeds.

Before allocating any new Cycle/Run, the preview counts all same-project Cycles within the current UTC day against the chosen frozen Brief's `max_cycles_per_day`. Exhaustion returns nonretryable 429/BUDGET_EXHAUSTED without new records. Original successful receipts replay before state/revision/quota checks; changing keys, pausing/resuming or selecting another Brief with the same budget does not reset the count. UTC midnight changes only the eligible date window, not history or ordinal numbering. One synchronous test uses [Vitest v4's native Date clock control](https://v4.vitest.dev/api/vi#vi-setsystemtime), restoring real time in `finally`; no custom clock framework or real-time wait is added. Browser acceptance also checks freeze revision advancement, exact original-key replay and a fresh GET of the same frozen resource.
...
| `6808e394585ca0d063442dd4234f8d8559a06719` | [Hosting 35185968521](https://github.com/zhengui666/QuaZonai/actions/runs/35185968521), job `105087996731`: real Caddy 2.11.4, **8 passed / 0 failed/skipped**, including existing static header parity and independent upstream headers; user-manager prerequisite, unit syntax and source checks passed | Controlled peers and `/usr/bin/true`, not QZ deployment or research |
| `6808e394585ca0d063442dd4234f8d8559a06719` | [Web 35185968555](https://github.com/zhengui666/QuaZonai/actions/runs/35185968555), job `105087996805`: all stages completed successfully, including contracts/type/unit/build, Demo, three-viewport/accessibility/PWA, real Rust API/TOTP/idempotency/database browser checks and unchanged source | Does not approve subsequent revision/CAS/quota corrections or establish full T42 acceptance |
