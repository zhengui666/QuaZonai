...
The owner's standing [authorization](../AGENTS.md#开发权限) permits a scoped merge after all applicable latest-Head CI succeeds, all review threads are resolved, and `@codex review` explicitly reports no issues on that Head. A reaction, silence, pending/cancelled/skipped/failed run, quota error or old review does not pass. GitHub Codex reviews only; the web assistant fixes through file tools and Actions verifies, as specified by [the owner amendment](../DESIGN.md#acceptance-scope). Any new commit restarts the applicable checks and review cycle. Use an expected-Head merge and reread main afterward.

Maintainer decisions are required for changed product requirements, significant architecture, license changes or production release; authors cannot manufacture approval. The owner explicitly waived dedicated-account acceptance in DESIGN 0.4: record COMPLETED_BY_OWNER_WAIVER with NOT_RUN, not PASSED, and do not demand the waived account or a CodexPro reconnection. This does not waive remaining checks, current-Head review or product authorization. The current task's explicit CI/review endpoint is recorded in [its intent](tasks/personal-production/task.md#task). No general requirement is inferred that an unrelated product Issue must close before a scoped maintenance PR merges.
