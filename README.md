...
| 原生封存计算 | 本机入口及内部受管EVALUATE_SEALED_ALPHA复用目录、Wasm、冻结校准和原生指标；保留原始分数、校准收益及缺失原因，转换为原资产scope指标。真实OCI执行已验证；首次能力返回前按原Attempt预约根血缘机会，取消不退款。可信任务创建、正式评估、独立Reviewer及资格尚未接通 |
