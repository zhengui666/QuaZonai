...
Run the migration and start the stack while it remains behind loopback or a trusted private network. On the first Web visit, the setup page generates a one-time QR/manual key, holds it only in the browser, and binds the first confirmed Google Authenticator-compatible 6-digit code into PostgreSQL. Complete this first claim before exposing the installation publicly; the first successful confirmation wins. The setup candidate expires after 10 minutes and can be regenerated. Existing deployments may set `QUAZONAI_AUTH_TOTP_SECRET` temporarily for the one-time startup importer, then remove it after verifying the durable binding.
...
TOTP-only is single-factor authentication and is weaker against online guessing than password + TOTP. Internet-facing deployments should still use HTTPS, narrowly scoped trusted-proxy configuration, and deployment-level network access controls. Non-empty deprecated browser username/password environment settings fail startup closed when authentication is enabled. The durable `OperatorAuthConfiguration` binding is canonical; it survives cookie/session expiry and API restarts. A missing binding is setup-required only on a healthy fresh installation; database/master-key/ciphertext failures fail closed and do not reopen setup.
...
After startup, open **Administration → Runtime configuration** to configure the Codex model, reasoning effort, independent Fast mode, optional custom OpenAI-compatible Base URL, optional API key, Worker limits, or **Sign in with ChatGPT**. Reasoning effort defaults to the selected model and accepts only `minimal`, `low`, `medium`, `high`, or `xhigh`; Fast requests Codex's native `service_tier="fast"` for new Missions and is not silently downgraded. These values are persisted in PostgreSQL instead of `.env`; Codex API keys and ChatGPT OAuth tokens are AES-GCM encrypted at rest by `QUAZONAI_MASTER_KEY` and never returned to the Web/API. ChatGPT Auth uses a Device Code flow; its state-changing start request requires a non-safelisted `application/json` body even when direct access is enabled, and `CODEX_HOME/auth.json` is only a one-time upgrade import source, never the runtime fact source. API and finite worker admission perform that import before consuming Missions, including custom-provider Missions. Device polling persists a longer interval after OpenAI requests `slow_down` and serializes concurrent exchanges with a durable in-flight lease whose release is owner-checked; start rechecks canonical auth after waiting on a pending attempt. Terminal authorization failures remain visible in the Web error panel. Readiness validates both stored token envelopes and persisted custom-provider API keys before reporting ready. Re-authentication after a revoked or corrupt ChatGPT credential creates a new canonical auth identity, while old Mission refresh callbacks remain bound to their original auth row/account and return the App Server's camelCase wire fields. Runtime configuration validation errors do not echo submitted provider keys. Disconnect removes that legacy file before committing database credential deletion; a cleanup failure leaves the database credentials unchanged. Runtime changes apply to newly claimed work without rebuilding the Compose stack.
