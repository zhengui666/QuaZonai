...
已安装 Node.js ≥22.12、npm 和 make 的开发环境可在仓库根目录运行 `make demo-preview`；该命令按锁文件安装前端依赖，然后在 `http://127.0.0.1:4179` 启动开发中的合成预览，按 Ctrl+C 停止。它复用正式界面及原生响应合同，展示冻结 Brief、无合格候选的周期与试验选择、两个 Alpha、冻结组合配置、执行假设、评估政策及资格不足的无目标候选；另有停用 Runtime、过期许可、投资域和按分区筛选的 FIXTURE 元数据，以及独立的过期 DEMO 目标包历史与原始附件下载。新增历史的 PASS、资格和权重均是假设展示，未执行计算，不构成真实研究证据。它允许临时编辑示例项目的名称和说明，保留版本冲突与原键重放；同一预览实例共享内存数据，重启清空，最多保存 256 次编辑回执，达到上限后需重启。状态变更、其他写入与 Claim 仍被拒绝，不连接真实 API、数据库或账号。完整交付与其余编辑交互仍待补齐，不能替代 T02/T42；请勿在预览中输入真实凭据。已有依赖时可直接运行 `npm --prefix apps/web run demo:preview`。
