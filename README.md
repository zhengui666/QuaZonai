...
After startup, open **Administration → Runtime configuration** to configure the Codex model, reasoning effort, independent Fast mode, optional custom OpenAI-compatible Base URL, optional API key, Worker limits, or **Sign in with ChatGPT**. Reasoning effort defaults to the selected model and accepts only `minimal`, `low`, `medium`, `high`, or `xhigh`; Fast requests Codex's native `service_tier="fast"` for new Missions and is not silently downgraded. These values are persisted in PostgreSQL instead of `.env`; Codex API keys and ChatGPT OAuth tokens are AES-GCM encrypted at rest by `QUAZONAI_MASTER_KEY` and never returned to the Web/API. ChatGPT Auth uses a Device Code flow; its state-changing start request requires a non-safelisted `application/json` body even when direct access is enabled, and `CODEX_HOME/auth.json` is only a one-time upgrade import source, never the runtime fact source. API and finite worker admission perform that import before consuming Missions. Device polling persists a longer interval after OpenAI requests `slow_down`; readiness validates both stored token envelopes and persisted custom-provider API keys before reporting ready. Disconnect removes that legacy file before committing database credential deletion; a cleanup failure leaves the database credentials unchanged. Mission refresh callbacks remain bound to the original auth row/account and return the App Server's camelCase wire fields. Runtime changes apply to newly claimed work without rebuilding the Compose stack.
