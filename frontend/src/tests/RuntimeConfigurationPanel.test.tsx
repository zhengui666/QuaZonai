...
describe('RuntimeConfigurationPanel directionality', () => {
  ...
  it('keeps device-code authorization in memory and exposes the fixed OpenAI page', async () => {
    const auth = {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation((input, options) => {
      ...
      if (path.endsWith('/device/start')) return jsonResponse({
        login_id: '00000000-0000-0000-0000-000000000050',
        status: 'PENDING',
      ...
      if (path.endsWith('/device/00000000-0000-0000-0000-000000000050')) return jsonResponse({ status: 'CANCELLED', expires_at: null, poll_after_seconds: null, auth: null, error_code: null });
    ...
    expect(await screen.findByText('ABCD-EFGH')).toHaveAttribute('dir', 'ltr');
    expect(screen.getByRole('link', { name: translateKey('en', 'runtime.deviceCodeOpen') })).toHaveAttribute('href', 'https://auth.openai.com/codex/device');
