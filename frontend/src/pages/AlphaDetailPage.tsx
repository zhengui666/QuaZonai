...
export function AlphaDetailPage() {
  ...
  const calibrationOption = useMemo(() => {
    ...
    return { grid: { left: 44, right: 18, top: 14, bottom: 32 }, tooltip: { trigger: 'axis' }, xAxis: { type: 'value', name: 'Predicted', min, max }, yAxis: { type: 'value', name: 'Observed', min, max }, series: [{ type: 'line', showSymbol: true, data: calibration.map((item) => [item.predicted, item.observed]), lineStyle: { color: '#4f9b82' }, itemStyle: { color: '#4f9b82' } }, { type: 'line', showSymbol: false, data: [[min, min], [max, max]], lineStyle: { color: '#7d8884', type: 'dashed' } }] };
  }, [calibration]);
