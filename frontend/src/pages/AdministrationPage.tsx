...
const datasetColumns: ColumnDef<DatasetRevision, unknown>[] = [
  ...
  { accessorKey: 'partition', header: 'Partition', meta: { messageKey: 'admin.partition', localizedSort: true }, cell: ({ getValue }) => <StateBadge state={String(getValue() ?? 'DISCOVERY')} /> },
  ...
  { accessorKey: 'quality_state', header: 'Quality', meta: { messageKey: 'admin.quality', localizedSort: true }, cell: ({ getValue }) => <StateBadge state={String(getValue() ?? 'UNKNOWN')} /> },
  { accessorKey: 'point_in_time_state', header: 'PIT', meta: { messageKey: 'admin.pit', localizedSort: true }, cell: ({ getValue }) => <StateBadge state={String(getValue() ?? 'UNKNOWN')} /> },
...
const universeColumns: ColumnDef<MarketUniverse, unknown>[] = [
  ...
  { accessorKey: 'state', header: 'State', meta: { messageKey: 'research.state', localizedSort: true }, cell: ({ getValue }) => <StateBadge state={String(getValue() ?? 'ACTIVE')} /> },
];
...
const pluginColumns: ColumnDef<PluginRelease, unknown>[] = [
  ...
  { accessorKey: 'state', header: 'State', meta: { messageKey: 'research.state', localizedSort: true }, cell: ({ getValue }) => <StateBadge state={String(getValue())} /> },
  { id: 'capabilities', header: 'Capabilities', meta: { messageKey: 'admin.capabilities', localizedSort: true }, cell: ({ row }) => <span className="qz-list-subtitle">{row.original.capabilities?.map(humanize).join(', ') || '—'}</span> },
...
function DataSourceDialog() {
  ...
  return <Dialog.Root><Dialog.Trigger><Button size="1" variant="soft"><DatabaseIcon size={14} />{t('admin.registerDataSource')}</Button></Dialog.Trigger><Dialog.Content maxWidth="500px"><Dialog.Title>{t('admin.registerGoverned')}</Dialog.Title><Dialog.Description size="2">{t('admin.dataSourceDesc')}</Dialog.Description><div className="qz-form-grid" style={{ marginTop: 18 }}><label className="qz-field"><span className="qz-label">{t('admin.name')}</span><TextField.Root value={name} onChange={(event) => setName(event.target.value)} /></label><label className="qz-field"><span className="qz-label">{t('admin.provider')}</span><TextField.Root value={provider} onChange={(event) => setProvider(event.target.value)} /></label><label className="qz-field"><span className="qz-label">{t('admin.canonicalFields')}</span><TextField.Root dir="ltr" value={fields} onChange={(event) => setFields(event.target.value)} placeholder="event_time, available_time, close, volume" /></label><Button disabled={!name.trim() || mutation.isPending} onClick={() => mutation.mutate({ name: name.trim(), provider: provider.trim(), fields: fields.split(',').map((value) => value.trim()).filter(Boolean), state: 'STAGED' })}>{mutation.isPending ? t('common.registering') : t('admin.register')}</Button></div></Dialog.Content></Dialog.Root>;
}
