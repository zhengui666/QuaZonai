...
const evaluationModelModes = ['RELATIVE_SCORE', 'CALIBRATED_RETURN'] as const;
...
const promotionPurposes = ['ALPHA_DISCOVERY_TO_SEALED', 'SEALED_TO_QUALIFIED', 'PORTFOLIO_TO_PAPER', 'PAPER_TO_LIVE'] as const;
  ...
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    ...
    <ResponsiveDialogContent maxWidth="760px">
      ...
      <div className="qz-form-grid" style={{ marginTop: 18 }}>
        ...
        <label className="qz-field"><span className="qz-label">Qualification metric code</span><TextField.Root dir="ltr" value={metricCode} onChange={(event) => setMetricCode(event.target.value)} /></label>
...
function PromotionPolicyVersionDialog({ downstreams }: { downstreams: ConfigurationDownstream[] }) {
  const mutation = useCreateConfigurationPromotionPolicyVersion();
  const [open, setOpen] = useState(false);
  const [purpose, setPurpose] = useState('');
  const [mode, setMode] = useState('');
  const [paperDownstreamId, setPaperDownstreamId] = useState('');
  const [liveDownstreamId, setLiveDownstreamId] = useState('');
  const [gates, setGates] = useState<PromotionGateForm[]>([]);
  const [error, setError] = useState<unknown>();
  const needsPaper = purpose === 'PORTFOLIO_TO_PAPER' || purpose === 'PAPER_TO_LIVE';
  const needsLive = purpose === 'PAPER_TO_LIVE';
  const paperDownstreams = downstreams.filter((downstream) => downstream.environment_type === 'PAPER');
  const liveDownstreams = downstreams.filter((downstream) => downstream.environment_type === 'LIVE');
  const selectPurpose = (value: string) => { setPurpose(value); setPaperDownstreamId(''); setLiveDownstreamId(''); };
  ...
  async function submit() {
    try {
      ...
      await mutation.mutateAsync({
        purpose,
        mode,
        paper_downstream_system_id: needsPaper ? paperDownstreamId : null,
        live_downstream_system_id: needsLive ? liveDownstreamId : null,
        gates: gatePayload,
        state: 'ACTIVE',
      });
      setOpen(false);
  ...
  const complete = purpose && mode && gatesComplete && (!needsPaper || paperDownstreamId) && (!needsLive || liveDownstreamId);
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    ...
    <ResponsiveDialogContent maxWidth="760px">
      ...
      <Dialog.Description size="2">Each gate is explicit and ordered. Downstream selectors only appear when the selected purpose requires the matching canonical Paper or Live consumer.</Dialog.Description>
      <div className="qz-form-grid" style={{ marginTop: 18 }}>
        <label className="qz-field"><span className="qz-label">Purpose</span><Select.Root value={purpose} onValueChange={selectPurpose}><Select.Trigger aria-label="Promotion purpose" placeholder="Select purpose" /><Select.Content>{promotionPurposes.map((value) => <Select.Item key={value} value={value}>{humanize(value)}</Select.Item>)}</Select.Content></Select.Root></label>
        ...
        {needsPaper ? <label className="qz-field"><span className="qz-label">Paper downstream</span><Select.Root value={paperDownstreamId} onValueChange={setPaperDownstreamId}><Select.Trigger aria-label="Paper downstream" placeholder="Select Paper downstream" /><Select.Content>{paperDownstreams.map((downstream) => <Select.Item key={downstream.id} value={downstream.id}>{downstream.name} · {downstream.id}</Select.Item>)}</Select.Content></Select.Root></label> : <label className="qz-field"><span className="qz-label">Downstream binding</span><TextField.Root dir="ltr" readOnly value="None for this purpose" /></label>}
        {needsLive ? <label className="qz-field"><span className="qz-label">Live downstream</span><Select.Root value={liveDownstreamId} onValueChange={setLiveDownstreamId}><Select.Trigger aria-label="Live downstream" placeholder="Select Live downstream" /><Select.Content>{liveDownstreams.map((downstream) => <Select.Item key={downstream.id} value={downstream.id}>{downstream.name} · {downstream.id}</Select.Item>)}</Select.Content></Select.Root></label> : null}
        <label className="qz-field"><span className="qz-label">State</span><TextField.Root dir="ltr" readOnly value="ACTIVE" /></label>
        {gates.map((gate, index) => <div className="qz-panel qz-panel-pad" key={index} style={{ gridColumn: '1 / -1' }}><div className="qz-list-title">Promotion gate {index + 1}</div><div className="qz-form-grid" style={{ marginTop: 12 }}><label className="qz-field"><span className="qz-label">Metric code</span><TextField.Root aria-label={`Gate ${index + 1} metric code`} dir="ltr" value={gate.metric_code} onChange={(event) => updateGate(index, 'metric_code', event.target.value)} /></label><label className="qz-field"><span className="qz-label">Comparator</span><Select.Root value={gate.comparator} onValueChange={(value) => updateGate(index, 'comparator', value)}><Select.Trigger aria-label={`Gate ${index + 1} comparator`} placeholder="Select comparator" /><Select.Content>{comparators.map((value) => <Select.Item key={value} value={value}>{humanize(value)}</Select.Item>)}</Select.Content></Select.Root></label><label className="qz-field"><span className="qz-label">Threshold</span><TextField.Root aria-label={`Gate ${index + 1} threshold`} dir="ltr" inputMode="decimal" value={gate.threshold} onChange={(event) => updateGate(index, 'threshold', event.target.value)} placeholder="Decimal string" /></label><label className="qz-field"><span className="qz-label">Ordinal</span><TextField.Root aria-label={`Gate ${index + 1} ordinal`} dir="ltr" inputMode="numeric" value={gate.ordinal} onChange={(event) => updateGate(index, 'ordinal', event.target.value)} /></label><Button size="1" color="red" variant="soft" onClick={() => setGates((current) => current.filter((_, gateIndex) => gateIndex !== index))}>Remove gate</Button></div></div>)}
