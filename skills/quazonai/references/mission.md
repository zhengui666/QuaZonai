...
3. Publish a permitted file using `artifact.submit`. Its fields are `schema_version`, `kind`, `workspace_relative_path` and `idempotency_key`. The kind is CODE, PARAMETERS or REPORT. The file is ordinary nonblank UTF-8, contains no NUL and is no larger than 2 MiB in the bound workspace; absolute, hidden and symlink paths are rejected. CODE contains source text. PARAMETERS and REPORT files must be JSON objects with numeric `schema_version: 1`, for example `{"schema_version":1,"text":"research notes"}`; a Markdown file or `{"schema_version":"1"}` is not a valid REPORT. Pass its relative filename, not the CLI's raw-content request wrapper. Do not provide `workspace_root`, project/Attempt overrides or secrets.
