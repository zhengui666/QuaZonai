...
The responsive Web/PWA client uses the same browser TOTP flow. Its offline shell never caches or exposes the setup secret, one-time code, session cookie, trusted-browser cookie, machine token, or API data; offline mode is only a presentation of the static shell and a server-connection notice.
