...
impl FeeModel for SettlementAwarePolymarketFee {
    fn get_commission(&self, order: &OrderAny, quantity: Quantity, price: Price,
        instrument: &InstrumentAny) -> Result<Money>
    {
...
pub(crate) fn native_payload(instrument: &InstrumentAny) -> Result<serde_json::Value> {
    ...
    value.get("BinaryOption").cloned().ok_or_else(|| anyhow::anyhow!("BINARY_OPTION_REQUIRED"))
}

pub(crate) fn validate_market(market: &NativeMarketData, settings: &NativeSimulationSettingsV1) -> Result<()> {
    ...
    for series in &market.series {
        match &series.instrument {
            InstrumentAny::BinaryOption(binary) => {
                ...
                let rate = settings.fee_rates.iter().find(|r| r.instrument_id == binary.id.to_string())
                    .ok_or_else(|| anyhow::anyhow!("SIMULATION_FEE_MISSING"))?;
                ensure!(rate.maker.as_decimal() == &bigdecimal::BigDecimal::from(0)
                    && rate.taker == ceiling, "POLYMARKET_PLANNING_FEE_SOURCE_MISMATCH");
                ensure!(series.bars.iter().all(|b| b.ts_event.as_u64() >= activation
                    && b.ts_event.as_u64() <= expiration), "POLYMARKET_BAR_OUTSIDE_CONTRACT_LIFETIME");
            }
...
pub(crate) fn close_events(root: &Path, market: &NativeMarketData, selection: &NativeBarSelectionV1)
    -> Result<Vec<InstrumentClose>>
{
    let binaries = market.series.iter().filter_map(|s| match &s.instrument {
        InstrumentAny::BinaryOption(b) => Some((b.id, b)), _ => None,
    }).collect::<BTreeMap<_, _>>();
    if binaries.is_empty() { return Ok(Vec::new()); }
    let mut catalog = ParquetDataCatalog::from_uri(root.to_str()
        .ok_or_else(|| anyhow::anyhow!("CATALOG_PATH_ENCODING"))?, None, Some(4096), None, None)?;
    ...
    for record in query.take(binaries.len() + 1) {
        let Data::InstrumentClose(close) = record? else { anyhow::bail!("CATALOG_NATIVE_TYPE_MISMATCH"); };
        let binary = binaries.get(&close.instrument_id).ok_or_else(|| anyhow::anyhow!("UNMAPPED_INSTRUMENT_CLOSE"))?;
        ensure!(close.ts_init >= close.ts_event
        ...
        if close.ts_init.as_u64() >= selection.event_end_ns.get() { continue; }
        ensure!(identities.insert(close.instrument_id), "DUPLICATE_POLYMARKET_SETTLEMENT");
        closes.push(close);
    }
...
pub(crate) fn order_notional(instrument: &InstrumentAny, now: u64, notional: Decimal) -> Result<()> {
    if let InstrumentAny::BinaryOption(binary) = instrument {
        ensure!(now >= binary.activation_ns.as_u64() && now < binary.expiration_ns.as_u64(),
            "POLYMARKET_ORDER_OUTSIDE_CONTRACT_LIFETIME");
        ensure!(notional >= Decimal::ONE, "POLYMARKET_RESEARCH_MINIMUM_NOTIONAL");
    }
    Ok(())
}
