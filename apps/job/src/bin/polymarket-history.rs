...
#[derive(Parser)]
#[command(version, about = "Prepare native Polymarket history; does not qualify or register research data")]
struct Arguments {
    #[command(subcommand)]
    command: Command,
}
...
fn epoch_ns(seconds: u64) -> Result<UnixNanos> {
    Ok(seconds.checked_mul(1_000_000_000).context("TIMESTAMP_RANGE")?.into())
}
...
fn historical_instrument(instrument: &InstrumentAny) -> Result<InstrumentAny> {
    ensure!(matches!(instrument, InstrumentAny::BinaryOption(_)), "BINARY_OPTION_REQUIRED");
    ensure!(instrument.venue().to_string() == "POLYMARKET", "POLYMARKET_VENUE_REQUIRED");
    let mut value = serde_json::to_value(instrument)?;
    if let Some(info) = value.get_mut("BinaryOption")
        .and_then(|v| v.get_mut("info")).and_then(serde_json::Value::as_object_mut)
    {
        for key in ["active", "closed", "closedTime", "closed_time", "resolved", "winner",
            "tokens", "outcomePrices", "outcome_prices", "umaResolutionStatus",
            "accepting_orders", "acceptingOrders", "gamma_market", "gamma_event"]
        {
            info.remove(key);
        }
    }
...
fn validate(archive: &NativeArchive) -> Result<()> {
    ensure!(!archive.source_reference.trim().is_empty() && archive.source_reference.len() <= 2000,
        "SOURCE_REFERENCE_REQUIRED");
    ensure!(archive.source_observed_at <= Utc::now(), "FUTURE_SOURCE_OBSERVATION");
    ensure!((1..=256).contains(&archive.instruments.len()), "INSTRUMENT_LIMIT");
    let rows = archive.trades.len().checked_add(archive.quotes.len())
        .and_then(|n| n.checked_add(archive.deltas.len()))
        .and_then(|n| n.checked_add(archive.bars.len())).context("ROW_COUNT_RANGE")?;
    ...
    for trade in &archive.trades {
        ...
        ensure!(valid_price(trade.price.as_decimal()) && trade.size.as_decimal() > Decimal::ZERO,
            "INVALID_BINARY_TRADE");
        ensure!(trade_ids.insert((trade.instrument_id, trade.trade_id)), "DUPLICATE_TRADE_ID");
        time_order(trade.ts_event, trade.ts_init)?;
    }
    for quote in &archive.quotes {
        ensure!(ids.contains(&quote.instrument_id), "UNMAPPED_QUOTE");
        ensure!(valid_price(quote.bid_price.as_decimal()) && valid_price(quote.ask_price.as_decimal())
            && quote.bid_price <= quote.ask_price, "INVALID_BINARY_QUOTE");
    ...
    for delta in &archive.deltas {
        ...
        ensure!(valid_price(delta.order.price.as_decimal()), "INVALID_BINARY_BOOK_PRICE");
    ...
    for bar in &archive.bars {
        ...
        ensure!(bar.bar_type.is_externally_aggregated()
            && bar.bar_type.spec().price_type == PriceType::Last, "ONLY_NATIVE_LAST_BARS");
        ensure!([bar.open, bar.high, bar.low, bar.close].iter().all(|p| valid_price(p.as_decimal()))
            && bar.low <= bar.open && bar.low <= bar.close && bar.high >= bar.open
            && bar.high >= bar.close && bar.high >= bar.low, "INVALID_BINARY_BAR");
...
fn import(mut archive: NativeArchive, output: &Path) -> Result<ImportReport> {
    ...
    fs::write(output.join("source-evidence.json"), serde_json::to_vec_pretty(&archive)?)?;
    let root = output.join("catalog");
    fs::create_dir(&root)?;
    let catalog = ParquetDataCatalog::from_uri(root.to_str().context("CATALOG_PATH_ENCODING")?,
        None, Some(4096), None, None)?;
    let instruments = archive.instruments.iter().map(historical_instrument).collect::<Result<Vec<_>>>()?;
    catalog.write_instruments(instruments)?;
    archive.trades.sort_by_key(|r| (r.ts_init, r.ts_event));
    ...
    if !archive.trades.is_empty() { catalog.write_to_parquet(&archive.trades, None, None, None)?; }
    if !archive.quotes.is_empty() { catalog.write_to_parquet(&archive.quotes, None, None, None)?; }
    if !archive.deltas.is_empty() { catalog.write_to_parquet(&archive.deltas, None, None, None)?; }
    if !archive.bars.is_empty() { catalog.write_to_parquet(&archive.bars, None, None, None)?; }
    ...
    fs::write(output.join("import-report.json"), serde_json::to_vec_pretty(&report)?)?;
    Ok(report)
}

fn read_archive(path: &Path) -> Result<NativeArchive> {
    let file = fs::File::open(path)?;
    ensure!(file.metadata()?.is_file() && file.metadata()?.len() <= MAX_INPUT_BYTES, "INPUT_FILE_LIMIT");
...
async fn fetch(slug: &str, start: u64, end: u64, max_trades: u32) -> Result<NativeArchive> {
    ensure!(!slug.is_empty() && slug.len() <= 240
        && slug.bytes().all(|b| b.is_ascii_alphanumeric() || b == b'-'), "INVALID_MARKET_SLUG");
    ensure!(start < end && end <= u64::try_from(Utc::now().timestamp())?, "INVALID_HISTORY_WINDOW");
    ...
    let observed_ns = UnixNanos::from(u64::try_from(observed.timestamp_nanos_opt().context("CLOCK_RANGE")?)?);
    ...
    for def in defs {
        ...
        let rows = client.request_trade_ticks(instrument.id(), def.condition_id.as_str(),
            def.token_id.as_str(), instrument.price_precision(), instrument.size_precision(),
            Some(start_ns), Some(epoch_ns(end - 1)?), Some(max_trades)).await?;
        // Enforce this command's half-open interval even if the upstream API returns a wider page.
        trades.extend(rows.into_iter().filter(|r| r.ts_event >= start_ns && r.ts_event < end_ns));
...
async fn main() {
    ...
    let result = match args.command {
        ...
        Command::Fetch { market_slug, start_seconds, end_seconds, max_trades, output } => {
            match fetch(&market_slug, start_seconds, end_seconds, max_trades).await {
                Ok(archive) => import(archive, &output), Err(error) => Err(error),
            }
        }
    };
    match result {
        Ok(report) => { println!("{}", serde_json::to_string(&report).expect("report serialization")); }
        Err(_) => { eprintln!("QZ_POLYMARKET_HISTORY_FAILED"); std::process::exit(1); }
    }
}
...
mod tests {
    use super::*;
    use nautilus_model::{
        enums::{AggressorSide, AssetClass}, identifiers::{InstrumentId, Symbol, TradeId},
        instruments::BinaryOption, types::{Currency, Price, Quantity},
    };
    use std::str::FromStr;

    fn archive() -> NativeArchive {
        let id = InstrumentId::from_str("test-condition-123456789012345678901234567890.POLYMARKET").unwrap();
        let instrument = BinaryOption::builder()
            .instrument_id(id).raw_symbol(Symbol::new("123456789012345678901234567890"))
            .asset_class(AssetClass::Alternative).currency(Currency::from_str("USD").unwrap())
            .activation_ns(0_u64.into()).expiration_ns(1000_u64.into())
            .price_precision(4).size_precision(6).price_increment(Price::from("0.0001"))
            .size_increment(Quantity::from("0.000001"))
            .ts_event(0_u64.into()).ts_init(0_u64.into()).build().unwrap();
        let trade = |time: u64, name: &str| TradeTick::new(id, Price::from("0.4200"),
            Quantity::from("2.000000"), AggressorSide::Buyer, TradeId::new(name), time.into(), (time + 1).into());
        NativeArchive {
            schema_version: SchemaV1, source_reference: "FIXTURE: native catalog round-trip".into(),
            ...
            quotes: Vec::new(), deltas: Vec::new(), bars: Vec::new(),
        }
    }
    ...
    fn native_records_round_trip_without_manufacturing_bars_or_qualification() {
        ...
        let rows = catalog.query::<TradeTick>(None, None, None, None, None, true).unwrap()
    ...
    fn invalid_or_duplicate_records_do_not_publish() {
        for mutation in 0..5 {
            ...
            match mutation {
                ...
                2 => data.trades[0].instrument_id = InstrumentId::from_str("other.POLYMARKET").unwrap(),
    ...
    fn existing_output_is_never_overwritten() {
        ...
        assert_eq!(fs::read(directory.path().join("owner-data")).unwrap(), b"preserve");
