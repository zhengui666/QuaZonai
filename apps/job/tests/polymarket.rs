...
use nautilus_model::{data::BarType, instruments::Instrument};
use prediction::{IDS, STEP};
use std::{fs, path::Path, str::FromStr};

fn simulation(rate: &str, resolved: Option<[&str; 2]>, delay_minutes: u64)
    -> (tempfile::TempDir, NativeSimulationRequestV1)
{
    ...
    target.targets = IDS.iter().map(|id| AllocationTargetV1 {
        instrument_id: (*id).into(), currency: "pUSD".into(), weight: "0.2".parse().unwrap(),
    }).collect();
    if let Some(payouts) = resolved {
        prediction::settle(directory.path(), expiration, (19 + delay_minutes) * STEP, payouts);
    }
    (directory, request)
}

fn simulate(root: &Path, request: &NativeSimulationRequestV1) -> Result<NativeSimulationResultV1, String> {
    let output = native::command(&["simulate".as_ref(), "--catalog".as_ref(), root.as_os_str()], request);
...
fn pnl(result: &NativeSimulationResultV1) -> f64 {
    result.statistics.iter().find(|s| s.group == NativeStatisticGroup::Pnls
        && s.native_key == "PnL (total)" && s.currency.as_deref() == Some("pUSD"))
        .unwrap_or_else(|| panic!("native statistics: {:?}", result.statistics)).value.unwrap()
}
...
fn native_shared_cash_settles_zero_one_and_half_at_original_availability() {
    for payouts in [["1.0000", "0.0000"], ["0.5000", "0.5000"]] {
        for delay in [0, 2 * 1440] {
            ...
            assert_eq!(result.summary.get("account.POLYMARKET.type").map(String::as_str), Some("CASH"));
            assert_eq!(result.summary.get("orders.open").map(String::as_str), Some("0"));
            // Both 500-share positions cost 400 pUSD and return 500 pUSD, sharing one account.
            assert!((pnl(&result) - 100.0).abs() < 0.0001, "{:?}", result.statistics);
            let canonical: nautilus_backtest::result::CanonicalBacktestResult =
                nautilus_backtest::result::CanonicalBacktestResult::from_slice(
                    &serde_json::to_vec(&result.canonical_result).unwrap()).unwrap();
...
fn native_price_dependent_commission_is_not_a_fixed_planning_coefficient() {
    ...
    assert!((pnl(&result) - 88.0).abs() < 0.0001, "{:?}", result.statistics);
...
fn original_fee_model_and_collateral_are_required_without_usd_aliases() {
    ...
    assert!(simulate(catalog.path(), &wrong).unwrap_err().contains("POLYMARKET_NATIVE_FEE_MODEL_REQUIRED"));
    let mut wrong = request;
    wrong.settings.base_currency = "USD".into();
    for point in &mut wrong.target_points { for target in &mut point.targets { target.currency = "USD".into(); } }
...
fn two_original_alpha_members_run_through_native_portfolio_study() {
    ...
    request.source_selection.bar_types = IDS.iter().map(|id| format!("{id}-1-MINUTE-LAST-EXTERNAL")).collect();
    for (asset, id) in request.assets.iter_mut().zip(IDS) { asset.instrument_id = id.into(); }
    assert!(request.members.len() >= 2);
    assert_ne!(request.members[0].alpha_version_id, request.members[1].alpha_version_id);
    let objects = tempfile::tempdir().unwrap();
    for member in &request.members { fs::write(objects.path().join(member.model_artifact_id.to_string()), &model).unwrap(); }
    fs::write(objects.path().join(request.mandate.constraints.transaction_costs_ref.to_string()),
        serde_json::to_vec(&request.execution_settings).unwrap()).unwrap();
    let output = native::command(&["study-portfolio".as_ref(), "--catalog".as_ref(), catalog.path().as_os_str(),
        "--objects".as_ref(), objects.path().as_os_str()], &request);
