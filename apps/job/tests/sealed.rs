...
#[test]
fn sealed_metric_projection_preserves_asset_scope_and_native_evidence() {
    use contracts::evidence::{Comparator, Decision, MetricRequirementV1};
    use domain::{
        evidence::evaluate_metrics,
        execution::{alpha_sealed_metrics, alpha_sealed_policy},
    };
    let (directory, request, frozen, wasm) = prepared();
    let report = evaluate_sealed_alpha(directory.path(), &request, &wasm, Some(&frozen)).unwrap();
    let evaluation = Id::new();
    let artifact = Id::new();
    let (metrics, capabilities) =
        alpha_sealed_metrics(evaluation, artifact, &request, Some(&frozen), &report).unwrap();
    assert_eq!(metrics.len(), 4);
    assert_eq!(capabilities.len(), 2);
    for (index, records) in metrics.as_chunks::<2>().0.iter().enumerate() {
        let points = &report.forecast.points[index * 25..(index + 1) * 25];
        let first = points.iter().find(|p| p.label_return.is_some()).unwrap();
        let last = points.iter().rfind(|p| p.label_return.is_some()).unwrap();
        for (record, native) in records.iter().zip(&report.assets[index].metrics) {
            assert_eq!(record.scope, format!("asset:{index}"));
            assert_eq!(record.value, native.value);
            assert_eq!(record.status, native.status);
            assert_eq!(record.reason_code, native.reason_code);
            assert_eq!(record.observation_count.get(), 21);
            ...
            assert_eq!(record.annualization_factor, None);
            assert_eq!(
                record.period_start.timestamp_micros(),
                (first.event_ns.get() / 1000) as i64
            );
            assert_eq!(
                record.period_end.timestamp_micros(),
                last.label_available_ns.unwrap().get().div_ceil(1000) as i64
            );
        }
        assert_eq!(records[0].unit, "CORRELATION");
        assert_eq!(records[1].unit, "RETURN_PER_HORIZON");
    }
    let requirements: Vec<_> = (0..2)
        .map(|index| MetricRequirementV1 {
            ...
            required: true,
            minimum_observations: count(21),
            method_allowlist: vec!["ndarray-stats.root_mean_sq_err".into()],
        })
        .collect();
    alpha_sealed_policy(&requirements, &request.forecast.selection, 2).unwrap();
    // Numeric gate only: synthetic provenance still forbids qualification.
    assert_eq!(
        evaluate_metrics(evaluation, &requirements, &metrics, &capabilities)
            .unwrap()
            .decision,
        Decision::Pass
    );
    for scope in ["asset:0/fold:0", "asset:2", "asset:00"] {
        let mut invalid = requirements.clone();
        invalid[0].scope = scope.into();
        assert!(alpha_sealed_policy(&invalid, &request.forecast.selection, 2).is_err());
        assert_eq!(
            evaluate_metrics(evaluation, &invalid, &metrics, &capabilities)
                .unwrap()
                .decision,
    ...
    let mut invalid = requirements.clone();
    invalid[0].method_allowlist = vec!["unimplemented".into()];
    assert!(alpha_sealed_policy(&invalid, &request.forecast.selection, 2).is_err());
    let mut empty = request.clone();
    empty.target_kind = TargetKind::ExpectedReturn;
    empty.forecast.selection.event_end_ns = count(29 * INTERVAL_NS);
    let report =
        evaluate_sealed_alpha(directory.path(), &empty, &module("f64.const 0.01"), None).unwrap();
    let (metrics, capabilities) =
        alpha_sealed_metrics(evaluation, artifact, &empty, None, &report).unwrap();
    for metric in &metrics {
        assert_eq!(metric.observation_count.get(), 0);
        assert_eq!(metric.value, None);
        assert_eq!(metric.status, MetricStatus::InsufficientData);
        assert_eq!(metric.reason_code.as_deref(), Some("NO_COMPLETE_LABELS"));
