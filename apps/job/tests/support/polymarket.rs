...
pub const IDS: [&str; 2] = [
    "fixture-event-101.POLYMARKET",
    "fixture-event-102.POLYMARKET",
];
...
pub fn write_catalog(root: &Path, rows: u32, rate: &str, expiration: u64, slope: bool) {
    let catalog =
        ParquetDataCatalog::from_uri(root.to_str().unwrap(), None, Some(4096), None, None).unwrap();
    catalog
        .write_instruments(instruments(rate, expiration))
        .unwrap();
        ...
        let records = (1..=rows)
            .map(|minute| {
                ...
                Bar::new_checked(
                    ...
                    price,
                    price,
                    Quantity::from("10000000.000000"),
                    (u64::from(minute) * STEP).into(),
                    (u64::from(minute) * STEP + 1).into(),
                )
                .unwrap()
            })
            .collect::<Vec<_>>();
        catalog
            .write_to_parquet(&records, None, None, None)
            .unwrap();
    }
}
...
pub fn settings(original: &mut NativeSimulationSettingsV1, rate: &str, expiration: u64) {
    ...
    original.fee_rates = instruments(rate, expiration)
        .iter()
        .map(|instrument| {
