...
test('Approval reads all decisions and retries the exact original intent', async ({ page }) => {
  ...
  expect(cursors).toContain(id(403)); expect(writes).toHaveLength(3); expect(writes[0]).toEqual(writes[1]); expect(writes[0]).toEqual(writes[2]);
...
test('Approval revocation keeps latest history and replays immediate intent', async ({ page }) => {
  ...
  await modal.getByRole('combobox', { name: '生效方式', exact: true }).click();
  await page.locator('.ant-select-dropdown:visible .ant-select-item-option-content').filter({ hasText: '指定时间' }).click();
  const date = modal.getByLabel('生效时间（本地时间）', { exact: true });
  const submit = modal.getByRole('button', { name: '确认追加撤销', exact: true });
  await date.fill('2100-01-01T00:00'); await expect(submit).toBeDisabled();
  await date.fill('2020-01-01T00:00'); await expect(submit).toBeDisabled();
  await date.fill('2098-01-01T00:00'); await expect(submit).toBeEnabled();
  expect(writes).toEqual([]);
  await modal.getByRole('combobox', { name: '生效方式', exact: true }).click();
  await page.locator('.ant-select-dropdown:visible .ant-select-item-option-content').filter({ hasText: '立即生效' }).click();
  await page.context().setOffline(true); await expect(submit).toBeDisabled();
  expect(writes).toEqual([]);
  await page.context().setOffline(false); await submit.click();
