...
use contracts::{
    ...
    runtime_jobs::{JobSpecV1, ResultManifestV1, RuntimeInputV1, RuntimeJobState, RuntimeResultState},
    settings::RuntimeUpdate,
    Id, SchemaV1,
};
use futures_util::FutureExt;
use integrations::{artifacts::ArtifactStore, authentication::random_capability, secrets::SecretVault};
...
impl Worker {
    fn start(binary: &Path, database: &str, state: &Path, origin: &str, log: PathBuf) -> Self {
        let file = fs::OpenOptions::new()
            .write(true).create_new(true).mode(0o600).open(&log).unwrap();
        ...
        let child = Command::new(binary)
            ...
            .env("RUST_LOG", "warn")
            .stdout(file.try_clone().unwrap()).stderr(file)
            .spawn().expect("built native Worker CLI must start");
        Self { child, log }
    }
    ...
    async fn deferred(&mut self) {
        tokio::time::timeout(Duration::from_secs(25), async {
            loop {
                ...
                if log.contains("worker task deferred for native reconciliation") { break; }
                tokio::time::sleep(Duration::from_millis(50)).await;
            }
        }).await.expect("missing local parameters must reach a real deferred Worker result");
    }

    async fn stop(&mut self) {
        self.alive();
        let sent = tokio::process::Command::new("kill")
            .env_clear().args(["-TERM", &self.child.id().to_string()])
        ...
        tokio::time::timeout(Duration::from_secs(10), async {
            loop {
                if let Some(status) = self.child.try_wait().unwrap() {
                    assert!(status.success(), "native Worker must exit normally on SIGTERM");
                    break;
                }
                tokio::time::sleep(Duration::from_millis(20)).await;
            }
        }).await.expect("owned Worker shutdown deadline");
    }
}
...
async fn expire(pool: &PgPool, attempt: Id) {
    ...
    ).bind(attempt.as_uuid()).fetch_one(pool).await.unwrap();
    assert!(expired);
}
...
async fn remote_bytes(runtime: &Fixture, spec: &JobSpecV1) -> (Vec<u8>, ResultManifestV1, Vec<Vec<u8>>) {
    let response = runtime.client.get(runtime.url(&["jobs", &spec.external_job_id, "result"]))
        .send().await.unwrap();
    ...
    for artifact in &manifest.artifacts {
        let response = runtime.client.get(runtime.url(&[
            "jobs", &spec.external_job_id, "artifacts", &artifact.storage_ref.to_string()
        ])).send().await.unwrap();
        assert_eq!(response.status(), reqwest::StatusCode::OK);
...
async fn verify_publication(
    pool: &PgPool, objects: &ArtifactStore, run: Id, owner: Id,
    raw: &[u8], manifest: &ResultManifestV1, outputs: &[Vec<u8>],
) {
    ...
    for (descriptor, bytes) in manifest.artifacts.iter().zip(outputs) {
        let matches: Vec<_> = mapped.iter().filter(|row| row.0 == descriptor.storage_ref.to_string()).collect();
        assert_eq!(matches.len(), 1);
        let row = matches[0];
        assert_eq!(row.2 as u64, descriptor.byte_count.get());
        assert!(objects.read(Id::try_from(row.1.clone()).unwrap(), descriptor.byte_count).unwrap() == *bytes);
    }
    ...
    assert!(objects.read(Id::try_from(stored.0).unwrap(), count(stored.1 as u64)).unwrap() == raw);
    let origins: Vec<String> = sqlx::query_scalar("SELECT origin FROM app.artifacts WHERE producer_run_id=$1")
        .bind(run.as_uuid()).fetch_all(pool).await.unwrap();
...
async fn checkpoint(
    pool: &PgPool, restored_name: &str, role: &str,
    database_attempted: &mut bool, role_attempted: &mut bool,
) {
    let binary = PathBuf::from(std::env::var_os("QUAZONAI_NATIVE_SERVER_BIN")
    ...
    let [series] = observed.series.as_slice() else { panic!("one controlled native instrument expected"); };
    let mut metadata = tasks::data::catalog_fixture::metadata();
    metadata.event_start = chrono::DateTime::from_timestamp_nanos(selection.event_start_ns.get() as i64);
    metadata.event_end = chrono::DateTime::from_timestamp_nanos(selection.event_end_ns.get() as i64);
    metadata.available_through = chrono::DateTime::from_timestamp_nanos(selection.decision_cutoff_ns.get() as i64);
    metadata.row_count = count(observed.rows as u64);
    metadata.universe.coverage_end = metadata.event_end;
    metadata.universe.instrument_definitions = vec![serde_json::to_value(&series.instrument).unwrap()];
    ...
    let mut config: serde_json::Value = serde_json::from_slice(&fs::read(&remote.config_path).unwrap()).unwrap();
    ...
    let response = remote.client.get(remote.url(&["catalogs", &metadata.registered_ref, "metadata"]))
        .query(&[("storage_version", &metadata.storage_version)]).send().await.unwrap();
    assert_eq!(response.status(), reqwest::StatusCode::OK);
    ...
    fs::DirBuilder::new().mode(0o700).create(state.join("secrets")).unwrap();
    ...
    data.runtime = data.store.update_runtime(
        &data.actor, "joint-native-runtime", data.runtime.id,
        &RuntimeUpdate { schema_version: SchemaV1, expected_revision: data.runtime.revision,
            configuration, credential_ref: Some(secret), ca_certificate_ref: None },
        |_| async { Ok(()) },
    ...
    let dataset = tasks::data::complete(&data, ticket, observed_metadata.clone()).await.unwrap().resource;
    let input = data.store.create_input_set(&data.actor, "joint-input", &InputSetCreate {
        schema_version: SchemaV1, project_id: data.project, purpose: InputPurpose::Discovery,
        decision_cutoff: metadata.available_through,
        items: vec![InputItemV1::Dataset { dataset_revision_id: dataset.id, role: DataPartition::Discovery }],
    }).await.unwrap().resource;
    let capabilities: RuntimeCapabilitiesV1 = remote.json(
        reqwest::Method::GET, &["capabilities"], None, &[reqwest::StatusCode::OK]
    ).await;
    tasks::probe(&data, capabilities.clone()).await;
    let f = tasks::Fixture { request: DataValidateRequest {
        schema_version: SchemaV1, project_id: data.project, input_set_id: input.header.id,
        runtime_id: data.runtime.id, expected_runtime_revision: data.runtime.revision,
        limits: JobLimitsV1 { schema_version: SchemaV1, experiments: 0, cpu_seconds: count(30),
            wall_seconds: 120, memory_mib: 512, output_bytes: count(65_536) },
    }, data, dataset, capabilities };
    let run = tasks::start(&f, "joint-original-run", &f.request).await.unwrap().resource;
    ...
    for input in &job.spec.inputs {
        if let RuntimeInputV1::Artifact { artifact_id, byte_count, storage_version, .. } = input {
            assert_eq!(storage_version, "1");
            remote.object(*artifact_id, &f.data.objects.read(*artifact_id, *byte_count).unwrap()).await;
        }
    }
    assert!(f.data.store.begin_run_dispatch(run.id, &lease.fence).await.unwrap());
    ...
    assert_eq!(facts(pool, run.id, message.message_id).await, (1, 0, 0, 1, 0));
    let dispatch: String = sqlx::query_scalar("SELECT dispatch_state FROM app.run_attempts WHERE id=$1")
    ...
    remote.crash();
    assert!(fs::metadata(remote.config.state_dir.join("journal.sqlite-wal")).unwrap().len() > 32);
    let dumped = tokio::time::timeout(Duration::from_secs(30),
        postgres::postgres_tool(pool, "pg_dump").args(["--format=custom", "--no-owner", "--no-privileges"]).output()
    ).await.expect("native database dump deadline").unwrap();
    assert!(dumped.status.success() && dumped.stderr.is_empty(), "native checkpoint dump failed");
    assert!(dumped.stdout.starts_with(b"PGDMP"));
    assert!(!state.join("master.key").exists(), "master key must remain outside the data archive");
    let retained_state = restore_directory(&state, backup.path(), "control").await;
    let retained_remote = restore_directory(remote.directory.path(), backup.path(), "runtime").await;
    let retained_catalog = restore_directory(catalog.path(), backup.path(), "catalog").await;

    *database_attempted = true;
    sqlx::query(sqlx::AssertSqlSafe(format!("CREATE DATABASE {restored_name} TEMPLATE template0")))
        .execute(pool).await.unwrap();
    let restored = sqlx::postgres::PgPoolOptions::new().max_connections(4)
        .connect_with(pool.connect_options().as_ref().clone().database(restored_name)).await.unwrap();
    let mut restore = postgres::postgres_tool(&restored, "pg_restore")
        .args(["--dbname", "", "--single-transaction", "--exit-on-error", "--no-owner", "--no-privileges"])
    ...
    let (written, restored_output) = tokio::time::timeout(Duration::from_secs(30), async {
        tokio::join!(async { stdin.write_all(&dumped.stdout).await?; stdin.shutdown().await }, restore.wait_with_output())
    }).await.expect("native database restore deadline");
    let restored_output = restored_output.unwrap();
    assert!(written.is_ok() && restored_output.status.success() && restored_output.stderr.is_empty(),
    ...
    assert_eq!(facts(&restored, run.id, message.message_id).await, (1, 0, 0, 1, 0));
    assert!(registration(&restored, f.dataset.id, f.request.input_set_id).await == original_registration);
    ...
    let cutover = tokio::time::timeout(Duration::from_secs(20),
        tokio::process::Command::new(&binary).args(["recover-access", "--recovery-id", &Id::new().to_string()])
            .env_clear().env("DATABASE_URL", owner_url.as_str()).kill_on_drop(true).output()
    ).await.expect("native restored-access cutover deadline").unwrap();
    assert!(cutover.status.success() && cutover.stderr.is_empty(), "native recovery access cutover failed");
    ...
    let Actor::Browser { login_id } = &f.data.actor else { panic!("controlled operator fixture expected"); };
    assert!(restored_store.browser_authority(*login_id).await.is_err());

    let password = random_capability();
    let ddl: String = sqlx::query_scalar("SELECT format('CREATE ROLE %I LOGIN PASSWORD %L',$1::text,$2::text)")
        .bind(role).bind(&password).fetch_one(pool).await.unwrap();
    *role_attempted = true;
    assert!(sqlx::query(sqlx::AssertSqlSafe(ddl.as_str())).execute(pool).await.is_ok());
    restored_store.migrate_with_application_role(Some(role)).await.unwrap();
    let application = restored.connect_options().as_ref().clone().username(role).password(&password).to_url_lossy();
    ...
    assert!(serde_json::to_value(reopened_manifest).unwrap() == serde_json::to_value(&manifest).unwrap());
    let response = remote.client.get(remote.url(&["catalogs", &metadata.registered_ref, "metadata"]))
        .query(&[("storage_version", &metadata.storage_version)]).send().await.unwrap();
    assert_eq!(response.status(), reqwest::StatusCode::OK);
    ...
    let parameter = state.join("artifacts").join(job.spec.parameters_artifact_id.to_string());
    ...
    let mut missing = Worker::start(&binary, application.as_str(), &state, &origin, backup.path().join("missing.log"));
    missing.deferred().await;
    assert_eq!(facts(&restored, run.id, message.message_id).await, (1, 0, 0, 1, 0));
    let deferred_owner = attempt(&restored, run.id).await;
    assert_eq!((&deferred_owner.0, deferred_owner.1, &deferred_owner.2),
    ...
    let mut worker = Worker::start(&binary, application.as_str(), &state, &origin, backup.path().join("recovered.log"));
    ...
    assert_eq!((&current.0, current.1, &current.2), (&original_attempt.0, original_attempt.1, &original_attempt.2));
    assert!(current.3 > deferred_owner.3);
    let frozen: serde_json::Value = sqlx::query_scalar("SELECT spec_json FROM app.run_native_attempts WHERE attempt_id=$1")
        .bind(lease.fence.attempt_id.as_uuid()).fetch_one(&restored).await.unwrap();
    assert!(frozen == original_spec);
    assert_eq!(facts(&restored, run.id, message.message_id).await, (1, 1, manifest.artifacts.len() as i64, 0, 1));
    let objects = Arc::new(ArtifactStore::open(&state.join("artifacts")).unwrap());
    verify_publication(&restored, &objects, run.id, lease.fence.attempt_id,
        &manifest_raw, &manifest, &output_bytes).await;
    ...
    assert_eq!(same.state.as_ref().unwrap().started_at, native.state.as_ref().unwrap().started_at);
    assert_eq!(same.state.as_ref().unwrap().finished_at, native.state.as_ref().unwrap().finished_at);
    assert_eq!(facts(pool, run.id, message.message_id).await, (1, 0, 0, 1, 0));
    assert_eq!(attempt(pool, run.id).await, original_attempt);
    assert!(serde_json::to_value(f.data.store.get_run(&f.data.actor, run.id).await.unwrap()).unwrap()
        == serde_json::to_value(original_run).unwrap());
    for (label, retained) in [("control", &retained_state), ("runtime", &retained_remote), ("catalog", &retained_catalog)] {
        archive::tar("joint-retained-after-adoption", "--compare", &backup.path().join(format!("{label}.tar")), retained).await;
    }
    ...
    let login = restored_store.login_with_verified_step(&snapshot, snapshot.database_now.timestamp()/30 + 1, false, None).await.unwrap();
    let actor = Actor::Browser { login_id: login.id };
    let reader = objects.clone();
    let writer = objects.clone();
    let next = restored_store.start_data_validation(&actor, "joint-new-restored-input-read", &f.request,
    ...
    let next_message = restored_store.read_run_messages(1, 100).await.unwrap()
        .into_iter().find(|message| message.run_id == next.id).expect("new genuine command must queue");
    let mut next_worker = Worker::start(&binary, application.as_str(), &state, &origin, backup.path().join("next.log"));
    adopted(&restored, next.id, next_message.message_id, &mut next_worker).await;
    next_worker.stop().await;
    let document: serde_json::Value = sqlx::query_scalar("SELECT spec_json FROM app.run_native_attempts WHERE run_id=$1")
        .bind(next.id.as_uuid()).fetch_one(&restored).await.unwrap();
    ...
    verify_publication(&restored, &objects, next.id, Id::try_from(next_attempt.0).unwrap(),
        &next_raw, &next_manifest, &next_outputs).await;
    let next_container = remote.native_container(&next_spec).await;
    assert_ne!(next_container.id, native.id);
    assert_eq!(remote.status(&job.spec).await, status);
    assert!(registration(&restored, f.dataset.id, f.request.input_set_id).await == original_registration);
    assert!(registration(pool, f.dataset.id, f.request.input_set_id).await == original_registration);
...
async fn restored_control_and_runtime_adopt_the_original_native_result(pool: PgPool) {
    ...
    let outcome = AssertUnwindSafe(tokio::time::timeout(Duration::from_secs(300),
        checkpoint(&pool, &database, &role, &mut database_attempted, &mut role_attempted)
    )).catch_unwind().await;
    // All owned child guards have dropped before removing only these fresh names.
    let database_clean = !database_attempted || sqlx::query(sqlx::AssertSqlSafe(
        format!("DROP DATABASE IF EXISTS {database} WITH (FORCE)")
    )).execute(&pool).await.is_ok();
    let role_clean = !role_attempted || sqlx::query(sqlx::AssertSqlSafe(
        format!("DROP ROLE IF EXISTS {role}")
    )).execute(&pool).await.is_ok();
    assert!(database_clean && role_clean, "owned native recovery resources failed cleanup");
    match outcome {
        Ok(Ok(())) => {}
