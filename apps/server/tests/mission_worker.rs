...
async fn lost_native_send_ack_is_not_retried_and_expired_turn_persists_cancel(pool: PgPool) {
    ...
    let deadline =
        sqlx::query_scalar("SELECT LEAST(clock_timestamp()+interval '10 seconds',$1::timestamptz)")
    ...
    assert!(
        expired,
        "the second poll must follow actual database expiry"
    );
    assert_eq!(
        connection
            .drive_turn(
