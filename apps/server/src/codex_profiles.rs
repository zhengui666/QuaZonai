...
) -> Result<Json<Page<CodexProfileViewV1>>, ApiError> {
...
) -> Result<Json<CodexProfileViewV1>, ApiError> {
...
pub async fn probe(
    ...
    Authority(actor): Authority,
    headers: HeaderMap,
    body: Result<Json<CodexProbeRequestV1>, JsonRejection>,
) -> Result<Json<CommandResult<CodexProbeViewV1>>, ApiError> {
    let request = json(body)?;
    let key = idempotency_key(&headers)?.to_owned();
    let store = state.store.clone();
    let deployment = state.codex_deployment.clone();
...
) -> Result<Json<CodexObservationV1>, ApiError> {
    let Query(query) = query.map_err(|_| ApiError::validation())?;
    Ok(Json(
        state
            .store
            .codex_observation(&actor, query.profile_id)
            .await?,
    ))
}
