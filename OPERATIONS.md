...
当前是Rust重写实施分支，不是完整可部署产品。旧后端、旧前端、旧Compose和旧插件平台已按所有者要求删除；不保留兼容服务。当前支持README的原生fixture/测试命令，不承诺Web或真实研究启动。

## 数据与权限

源码删除不授权删除运行中的数据库、artifacts、备份、Codex profile或宿主用户数据。不要在旧数据库运行新schema；未来切换使用独立数据库/卷，并按DESIGN完成只读导入报告和回滚演练。
