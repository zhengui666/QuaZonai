...
编写原子准备入口，科学任务及同Thread结果接续仍在实现，不能手工补库宣称完成。
...
仍不等于分折评估、独立Reviewer或资格；原Thread结果反馈及完整结论仍待接通。
...
研究Mission首轮请求使用冻结Brief与同Cycle剩余token额度，不自动选择另一Profile或扩大预算。完整原生用量结算后可使用剩余额度；结果未知时仍占用原预约，重试不会换请求或再插一轮。设置了费用上限但原生计费不可用时停止首轮准备，不假造价格。常驻Worker可显式启用Mission消费（完整参数见CLI）；它以独立容量领取、续约、准备首轮并驱动原生账本。首轮准备/模型回复都不等于科学任务、Mission或Cycle完成；结算后消息仍保留。就绪编译/预测可自动排队，但结果接续与结论阶段仍在开发。
