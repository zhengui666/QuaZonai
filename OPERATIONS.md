...
当前支持方差/CVaR下的最小风险/最大效用，以及方差风险预算；完整Candidate交付尚未验收。
选择“方差风险预算”后，明确填写风险资产总敞口，以及原资产标识、份额和LONG/SHORT
目标方向；份额合计1，不是资本权重。只做多不能分配正份额给SHORT。需要新镜像
portfolio-risk-budget/1与SECOND_ORDER_CONE能力。全部既有约束仍有效，冲突时不
生成近似替代目标。CVaR风险预算尚未支持；保存配置也不等于预算已可行或可交付。
