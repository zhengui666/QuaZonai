...
在 Actions → **Dev image** → **Run workflow** 中保持工作流分支为 `main`，在 `source_ref` 填入已推送到本仓库的开发分支、tag 或完整 commit SHA。源码无需包含 dev 工作流本身，但必须已有容器 action 和部署文件。也可执行以下命令，将示例分支替换为实际分支：

```sh
gh workflow run dev-image.yml --repo zhengui666/QuaZonai --ref main \
  -f source_ref=your-development-branch
```

工作流固定实际检出的 SHA，运行现有容器构建与真实安装/更新/恢复检查，成功后发布 `ghcr.io/zhengui666/quazonai:dev-<完整SHA>-<run-id>-<attempt>`。运行 Summary 提供来源 SHA、镜像 tag 和拉回核验过的 digest；后续拉取使用其中的完整 digest。重复运行及重试产生不同标签，开发分支继续提交不改变已完成运行的来源。私有 GHCR 包仍需 Docker 登录。
...
研究员和独立审阅员两个角色自动建立，共用同一 ChatGPT Codex 账号，使用独立任务和 Thread。
在“设置 → Codex → 共享 ChatGPT 账号”登录一次即可供两个角色使用；退出会影响所有角色。
下方“角色模型设置”可切换角色，分别保存模型、推理强度与标准／加速速度。
开启本机默认保留该角色已保存的值但不发送覆盖；关闭时发送非空模型／强度覆盖，
并明确应用速度选择。标准会关闭该角色的原生默认加速，不修改共享配置或另一个角色。
模型目录、强度和可选加速来自原生检测，不猜测或静默替换；“当前速度”显示检测到的结果。
...
原生Codex的Turn列表可能把断流失败重建为Completed，不能据此认定成功。QZ只以真实终态通知或已经保存的同一通知确认结果；丢失通知且没有记录时保留UNKNOWN/预约，列表“已完成”不触发自动结算、退款或重发。
...
Mission使用不同于科学任务的队列选择，但共用现有PGMQ、Run/Attempt和租约。原生Thread回执一旦绑定不能替换，原生创建应答未知时不盲目新建；首轮预约不等于模型已经RUNNING。首轮、上述科学反馈、独立Reviewer、封存评估和会话收束已接常驻Worker。专用账号部分的关闭依据是[所有者豁免](DESIGN.md#acceptance-scope)，不是代码接通或受控响应；真实市场数据、科学与交付全链仍须相应证据，不应手工改库补成功状态。
