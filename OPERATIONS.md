...
政策登记不等于Worker已运行或交付成功。当前Worker自动化只消费已有Release进行Paper/Live授权及交付，并处理Forward/Wake；
尚未按调仓日程自动创建Build、Study和新Release。新cutoff目前需通过上述原生构建、
独立Study和冻结目标包操作推进，不能把启用政策解释为全自动再平衡已交付。原政策行可“撤销政策”：读取原撤销历史，绑定最新记录追加立即或预约撤销，
