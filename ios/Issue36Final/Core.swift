...
public enum JSONValue: Codable, Hashable, Sendable {
    ...
    public var searchableText: String {
        ...
        case let .array(value):
            return value.map(\.searchableText).joined(separator: " ")
        default:
            return displayText
        }
    }
...
public enum APIError: Error, LocalizedError, Sendable, Equatable {
    ...
    public var errorDescription: String? {
        switch self {
        case .invalidServer: return "The server URL is invalid."
        case .insecureServer: return "Production connections require HTTPS."
        case .offline: return "The server is unavailable."
        case .unauthorized: return "Operator authentication is required."
        case let .conflict(_, message): return message
        case let .validation(_, message): return message
        case let .server(_, _, message): return message
        case .invalidResponse: return "The server returned an invalid response."
        case .missingCredential: return "No trusted-device credential is available."
        case .capabilityUpgradeRequired: return "A newer app version is required by this server."
        }
    }
    ...
    public var statusCode: Int? {
        ...
        case let .server(status, _, _): return status
        default: return nil
        }
    }
}
...
public final class CachedResource {
    ...
    public init(profileID: UUID, cacheKey: String, payload: Data, updatedAt: Date = .now) {
        compoundKey = "\(profileID.uuidString)|\(cacheKey)"
...
public actor EventStreamActor {
    ...
    public func start(baseURL: URL, accessToken: String?, cursor: Int) -> AsyncStream<EventStreamOutput> {
        task?.cancel()
        let pair = AsyncStream<EventStreamOutput>.makeStream()
        ...
        while !Task.isCancelled {
            do {
                ...
                try? await Task.sleep(
                    nanoseconds: UInt64(backoff) * 1_000_000_000 + jitter
                )
...
public struct ClientBootstrap: Equatable, Sendable {
    ...
    public init(json: JSONValue) throws {
        ...
        operatorClientCapabilityEpoch = Int(operatorEpoch)
        minimumIOSCapabilityEpoch = Int(minimumEpoch)
        minimumIOSAppVersion = minimumVersion
    }
}
...
public enum AppDestination: String, CaseIterable, Identifiable, Hashable {
    ...
    public var localizationKey: LocalizedStringResource {
        ...
        case .idea: return "nav.idea"
        case .research: return "nav.research"
        case .alpha: return "nav.alpha"
        case .portfolio: return "nav.portfolio"
        case .approvals: return "nav.approvals"
        case .handoff: return "nav.handoff"
        case .administration: return "nav.admin"
        case .security: return "nav.security"
...
public final class AppModel: ObservableObject {
    ...
    public init(defaults: UserDefaults = .standard) {
        ...
        appearance = AppearanceMode(
            rawValue: defaults.string(forKey: "issue36.appearance") ?? "system"
        ) ?? .system
        localeIdentifier = defaults.string(forKey: "issue36.locale") ?? Locale.current.identifier
        if let stored = defaults.string(forKey: "issue36.installationID"),
    ...
    public func login(totpCode: String, trustDevice: Bool) async throws {
        ...
        guard code.count == 6, code.allSatisfy(\.isNumber), let profile else {
            throw APIError.validation(
                code: "TOTP_FORMAT",
                message: String(localized: "auth.invalid")
            )
        }
        ...
        if trustDevice,
           let refresh = response.firstString(for: ["refresh_credential", "refresh_token"])
        {
            try keychain.saveRefreshCredential(refresh, profileID: profile.id)
        } else {
            try? keychain.deleteRefreshCredential(profileID: profile.id)
        }
        phase = .ready
        bannerMessage = nil
        await startEventStream()
    ...
    private func rotateTrustedCredential() async throws {
        ...
        guard let refresh = try keychain.readRefreshCredential(
        ...
        let access = try accessCredential(from: response)
        await api.setAccessToken(access)
        guard let rotated = response.firstString(
            for: ["refresh_credential", "refresh_token"]
        ) else { throw APIError.invalidResponse }
        try keychain.saveRefreshCredential(rotated, profileID: profile.id)
    }

    private func accessCredential(from response: JSONValue) throws -> String {
        guard let access = response.firstString(for: ["access_token", "access_credential"]),
              !access.isEmpty
        else { throw APIError.invalidResponse }
        return access
    }
    ...
    public func logout() async {
        ...
        if let profile { try? keychain.deleteRefreshCredential(profileID: profile.id) }
    ...
    private func profileForURL(_ url: URL) -> ServerProfile {
        ...
        let id: UUID
        if let stored = defaults.string(forKey: mappingKey),
    ...
    private func startEventStream() async {
        ...
        do {
            ...
            let cursor: Int
            if let modelContext {
                cursor = (try? OfflineCache.cursor(
                    profileID: profile.id,
                    context: modelContext
                )) ?? 0
            } else {
            ...
            let outputs = await eventStream.start(
                baseURL: baseURL,
                accessToken: token,
                cursor: cursor
            )
