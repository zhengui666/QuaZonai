        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    TextField("setup.server", text: $model.serverInput, prompt: Text("https://quazonai.example.com"))
                    ...
                    .controlSize(.large)
                    .disabled(model.serverInput.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    if let host = model.profile?.baseURL.host {
                        Text(host).font(.callout.monospaced()).foregroundStyle(.secondary)
                    }
...
private struct MoreView: View {
    private let destinations: [AppDestination] = [
        .idea, .alpha, .handoff, .administration, .security, .settings,
    ]
    ...
    var body: some View {
        List(destinations) { destination in
            NavigationLink {
                ...
                Label(String(localized: destination.localizationKey), systemImage: destination.symbol)
            }
        }
...
private struct IPadWorkbenchView: View {
    ...
    var body: some View {
        NavigationSplitView {
            List(destinations, selection: $model.destination) { destination in
                Label(String(localized: destination.localizationKey), systemImage: destination.symbol)
                    .tag(destination)
