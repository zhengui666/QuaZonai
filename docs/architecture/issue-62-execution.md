...
| [PR #93](https://github.com/zhengui666/QuaZonai/pull/93), `118863e8b6a44defa2f209a79864ffe595dc23ad` | Actual Optimal/Infeasible allocation reports, MCP/HTTP publication and explicit App Server restart retaining the original Thread, report bytes and history. Exact accepted checks and merge evidence remain in the PR. |
| [PR #94](https://github.com/zhengui666/QuaZonai/pull/94), `a6686641ade4fc2b2204605eb8aa2c7d4a292236` | Connected actual Validation, independent Reviewer PASS/REJECT and original Sealed execution. Exact accepted Head2b0b2c0 passed all five workflows and seven resolved review findings; controlled-data science grants no qualification. |
...
| T08: connected native scientific feedback | [Research target](../../apps/runtime/tests/native_research_loop.rs), [catalog](../../apps/runtime/tests/support/scientific_catalog.rs), [connected scenario](../../apps/runtime/tests/support/scientific_feedback.rs) | Actual computation and original Thread with a controlled Provider; not live model reasoning or full T42. PR #92 owns the accepted original rejection path; PR #94 verifies its independent Reviewer extension. |
| T08/T09: original science through independent review | [Connected scenario](../../apps/runtime/tests/support/scientific_feedback.rs), [native Reviewer fixture](../../apps/server/tests/support/codex_scientific_review.rs) | Engineered positive-control PASS/REJECT branches and actual Sealed execution accepted in PR #94. FIXTURE/PIT UNVERIFIED can never acquire production qualification. |
| T08: allocation-report consumption after restart | [Native science Thread](../../apps/server/tests/native_science_thread.rs) with `server/native-science` | Real Optimal/Infeasible Job reports, MCP/HTTP and explicit App Server restart accepted in PR #93; direct Job calls do not replace the production portfolio workflow. |
...
| UI/PWA/real entry | [Browser suites](../../apps/web/tests), [hosted restart](../../apps/web/scripts/native-browser.mjs) | Real local-session/project/receipt persistence is not the complete business workflow. |
...
| T41: shipped API/Worker user units | [Hosted runner](../../apps/web/scripts/native-browser.mjs), [native unit adapter](../../apps/web/scripts/native-user-services.mjs), [runbook](../user-guide.md#verify-changes-to-the-hosting-boundary) | Current deployment acceptance work: runtime-only copies/path overrides, idle Worker automatic restart and original API session/receipt. Check current PR results; no owner-host/public TLS/boot/active-job claim. |
...
[PR #93](https://github.com/zhengui666/QuaZonai/pull/93) accepted `cc0a800c9c73869c4cad00b3191a17b8a84b3772` after all five workflows and clean review5739302356. [Runtime35420630430](https://github.com/zhengui666/QuaZonai/actions/runs/35420630430), artifact10577915136, executed the actual Optimal/Infeasible report/restarted-Thread case and four parser controls alongside all prior native suites. [CI35420630439](https://github.com/zhengui666/QuaZonai/actions/runs/35420630439) retains688Store/Server and6native protocol passes. Bound merge118863e8 has no source diff; post-merge results are read separately in the PR.

Both summaries cite their actual immutable reports, and the second cannot reuse first-case weights. The original b2e80e9 candidate's mistaken `Final output:` parse and subsequent native `Output:` correction remain in that PR. The shared [exec parser](../../apps/server/tests/support/codex_tool_output.rs) preserves the same accepted format; existing controls remain. Direct Job execution does not replace the production portfolio workflow.

### Independent review of original native science

The [new field-level plan](https://github.com/zhengui666/QuaZonai/issues/62#issuecomment-5739701572) connects actual passing Validation to a different Reviewer Thread, then either an independent rejection or actual automatic Sealed execution. A causal reciprocal-close source on the authored linear-price catalog is an engineered positive control; the original policy and provenance stay unchanged. The native code, forecasts, raw report and Evaluation must really support PASS before review. Provider decisions alone are controlled; no science score is preset.

The actual Reviewer must read original materialized code/parameters/Validation context and retain original metric/report/policy/input identity. REJECT creates no Sealed job; PASS must execute original Sealed inputs through Runtime/Worker. Neither may rewrite Validation or grant qualification to FIXTURE data. [PR #94](https://github.com/zhengui666/QuaZonai/pull/94) accepted exact Head `2b0b2c0a2c4ec693fe8ee2b866c3f8c6eea1362a`: [Runtime35434667885](https://github.com/zhengui666/QuaZonai/actions/runs/35434667885), artifact10582178726, passed17OCI+2cold/ownership+1joint+6research+5science-thread. Both positive controls produced actual Validation PASS with160observations; independent PASS executed Sealed with observed PASS, while independent REJECT produced no Sealed. Full context, distinct profiles/Threads, exact scopes, original model/calibration/inputs, private report provenance, immutable Validation and unique receipts all passed. Qualification remained false. All five workflows,688Store/Server and explicit clean review5740783341 with seven resolved threads preceded mergea6686641; post-merge checks are recorded separately, not inferred. Existing independent protocol tests and the original rejection pipeline remain required.
...
| T41 | Non-account runbook/deployment coverage remains partial. Current work executes shipped user-unit API/Worker supervision through the real browser harness; its exact-Head result is required. Native help/Demo/loopback unit restart do not prove public TLS/host boot. Do not require the waived account example or CodexPro. |
