...
| T08: connected native scientific feedback | [Research target](../../apps/runtime/tests/native_research_loop.rs), [catalog](../../apps/runtime/tests/support/scientific_catalog.rs), [connected scenario](../../apps/runtime/tests/support/scientific_feedback.rs) | Actual computation and original Thread with a controlled Provider; not live model reasoning or full T42. PR #92 owns the accepted original rejection path; the new Reviewer extension requires separate execution. |
| T08/T09: original science through independent review | [Connected scenario](../../apps/runtime/tests/support/scientific_feedback.rs), [native Reviewer fixture](../../apps/server/tests/support/codex_scientific_review.rs) | New engineered positive-control PASS/REJECT branches and actual Sealed execution are pending verification, not declared passed. FIXTURE/PIT UNVERIFIED can never acquire production qualification. |
...
The actual Reviewer must read original materialized code/parameters/Validation context and retain original metric/report/policy/input identity. REJECT creates no Sealed job; PASS must execute original Sealed inputs through Runtime/Worker. Neither may rewrite Validation or grant qualification to FIXTURE data. These new branches have not executed at authorship; require the active PR's complete exact-Head native result and independent review before marking them accepted. Existing independent protocol tests and the original rejection pipeline remain required.
...
| T08 non-account connected loop | **Declared Worker/scientific feedback scope accepted in PR #92 and supplementary report/restart in PR #93.** The new independent Reviewer/Sealed connection is pending execution. Controlled Provider and FIXTURE data remain unqualified; no account scope is reopened. |
