...
sudo install -d -o root -g root -m 0755 /etc/quazonai
sudo install -o root -g root -m 0600 deploy/quazonai.env.example \
...
Fill `DATABASE_URL` locally with the non-owner application identity and set `PUBLIC_URL` to the browser's exact HTTPS origin. The sample intentionally has no database password and cannot start unchanged. The systemd manager reads the root-owned environment file before changing user; the application does not need permission to read that file itself. Do not `source` it: its syntax is systemd's `EnvironmentFile`, not a shell script.
...
sudo install -m 0644 deploy/systemd/quazonai-api.service \
  deploy/systemd/quazonai-worker.service /etc/systemd/system/
sudo systemd-analyze verify /etc/systemd/system/quazonai-api.service \
  /etc/systemd/system/quazonai-worker.service
sudo systemctl daemon-reload
sudo systemctl enable --now quazonai-api.service quazonai-worker.service
```

These services run the actual foreground commands, use native SIGTERM handling and restart on failure. They do not initialize keys, migrate the database, grant permissions or manufacture research results. An exhausted stop timeout can terminate local processes; it does not prove a remote task was cancelled. Use the existing Attempt/reconciliation procedure after interruption.
...
| `/etc/quazonai/quazonai.env` | Local service configuration and application DB identity | Administrator; root-owned 0600 |
...
**Upgrade:** prepare a second reviewed release, preserve the existing selection, stop the Worker and API in a controlled maintenance window, and take the database/artifact/key backups specified in [OPERATIONS](../OPERATIONS.md). Apply only explicitly reviewed migrations with the migration identity. Select the new release using an atomic symlink replacement, restart both services and repeat the real-host checks above. Allow the browser's prompted update or reload after checking outstanding commands. This is a bounded-downtime personal installation, not a claim of zero-downtime deployment.
