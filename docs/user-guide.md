...
Supply the disposable PostgreSQL administrator and build/browser prerequisites in [CONTRIBUTING](../CONTRIBUTING.md#verify-the-change), never an installation's database. The test creates its own application identity and state, enrolls through real TOTP, preserves the original browser session privately, stops the API normally, observes a real gateway 502 while static files remain available, and starts a new API process with the same database and keys. The second browser phase must read the original project and replay its original command receipt without duplication or another login, then revoke that session through logout. A missing Caddy executable, lost state, failed phase or cleanup is a failure, not a fallback to Vite or a skipped pass. Raw browser state and credentials are not uploaded.

This loopback scenario disables only automatic certificate issuance and the Caddy administrator listener; it does not issue a public certificate, start the installed systemd units, reboot the host, restore an active Worker or execute research. Inspect the actual accepted-Head workflow result before treating the test as evidence. None of these checks is a production deployment.
